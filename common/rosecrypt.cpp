...
//-------------------------------------------------------------------------
bool ctValueExistsInTable(CCryptTable *CryptTables, unsigned char Table,
                          unsigned Value, unsigned short EntryCount) {
...
void GenerateCryptTables(CCryptTable *&CryptTables, unsigned ModValue) {
	...
	CryptTables->AddTable		= (unsigned short *)calloc(0x200,
...
int DecryptBufferHeader(CCryptStatus *ri, CCryptTable *CryptTables,
	...
	TempBuffer.AddTableValue	 = ((Buffer[4] >> 0x03) & 0x0007) | ((
	                                   BigBuffer) & 0x0038) | \
	...
	TempBuffer.Command		 	 = ((BigBuffer >> 0x0C) & 0x0007) | ((
	                                   BigBuffer << 0x03) & 0x0038) | \
	                               ((Buffer[4] << 0x06) & 0x01C0) | ((BigBuffer >> 0x13) & 0x0600);
	TempBuffer.AddBufferLen		 = ((BigBuffer >> 0x06) & 0x0007) | ((
	                                   BigBuffer >> 0x0F) & 0x0038) | \
	                               ((BigBuffer >> 0x11) & 0x01C0) | ((Buffer[4] << 0x03) & 0x0600);
	TempBuffer.EncryptAddValue	 = ((BigBuffer >> 0x1E) & 0x0003) | ((
	                                   BigBuffer >> 0x13) & 0x000C);
...
void EncryptBuffer(CCryptTable *CryptTables, unsigned char *Buffer) {
	...
	TempBuffer.EncryptValue		 = TempBuffer.AddTableValue +
	                               TempBuffer.EncryptAddValue;
	...
	*((unsigned *)Buffer)		 = EncryptValue;
	...
	for (CheckSum = 0, c = 0; c < 5; ++c) {
		CheckSum   = CheckSumTable      [((unsigned char *)&TempBuffer)[ c ] ^
		                                 CheckSum ];
		Buffer[c] ^= CryptTables->Tables[ c ][ TempBuffer.AddTableValue ];
	}
	...
	Buffer[ 4 ]				 = (Buffer[4] & 0xC7) | (((unsigned)TempBuffer.AddTableValue) &
	                           0x38);
	*((unsigned *)Buffer)	 = EncryptValue;
}
