...
bool CCharServer::SendClanInfo(CCharClient *thisclient) {
	if (thisclient->clanid > 0) {
		...
		if (thisclan != NULL) {
			...
			ADDBYTE(pak,
			        thisclient->clan_rank);     // Clan rank (0 = red rokie / 6 = master)
...
bool CCharServer::pakClanManager(CCharClient *thisclient, CPacket *P) {
	...
	switch (action) {
		...
		case 0x03: { //Kick from clan
			...
			if (!DB->QExecute("UPDATE characters SET clanid=0 AND clan_rank=1 WHERE char_name='%s'",
			                  nick)) {
				delete []nick;
				return false;
			}
		...
		case 0x04: { //up rank
			...
			if (otherclient != NULL) {
				...
				online = false;
				result = DB->QStore("SELECT clan_rank,classid,level FROM characters WHERE char_name='%s'",
				                    nick);
			...
			if (!DB->QExecute("UPDATE characters SET clan_rank=%i WHERE char_name='%s'",
			                  clan_rank, nick)) {
				delete []nick;
				return false;
			}
		...
		case 0x06: { //Change news
			...
			if (thisclan != NULL) {
				...
				if (!DB->QExecute("UPDATE list_clan SET news='%s' WHERE id=%i", thisclan->news,
				                  thisclan->id)) {
					return false;
				}
			}
		...
		case 0x07: { //Leave Clan
			...
			if (!DB->QExecute("UPDATE characters SET clanid=0 , clan_rank=0 WHERE char_name='%s'",
			                  thisclient->charname)) {
				return false;
			}
		...
		case 0x09: { //Give Master
			...
			if (otherclient != NULL) {
				...
				//   0     1
				result = DB->QStore("SELECT classid,level FROM characters where char_name='%s'",
				                    nick);
		...
		case 0x0a: { //disorganize clan
			...
			if (!DB->QExecute("UPDATE characters SET clanid=0 AND clan_rank=1 WHERE clanid=%i",
			                  thisclient->clanid)) {
				return false;
			}
		...
		case 0x0c: { //invitation accepted
			...
			if (otherclient != NULL) {
				...
				if (!DB->QExecute("UPDATE characters SET clanid=%i , clan_rank=1 WHERE char_name='%s'",
		...
		case 0xfa: { //message from worldserver to load the new clan information
			...
			//     0    1  2    3    4   5   6       7
			result = DB->QStore("SELECT id,logo,back,name,cp,grade,slogan,news FROM list_clan where id=%i",
			                    id);
...
bool CCharServer::pakUploadCM(CCharClient *thisclient, CPacket *P) {
	...
	fclose(fh);
	DB->QExecute("UPDATE list_clan SET logo=%i,back=0 WHERE id=%i", thisclan->logo,
	             thisclient->clanid);
...
bool CCharServer::pakDownloadCM(CCharClient *thisclient, CPacket *P) {
	...
	MYSQL_RES *result = DB->QStore("SELECT logo FROM list_clan WHERE id=%i",
	                               clanid);
