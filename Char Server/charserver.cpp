...
bool CCharServer::OnServerReady() {
	...
	if (!DB->QExecute("INSERT INTO channels (id,name,host,port,connected,maxconnections,owner) VALUES (%i,'%s','%s',%i,0,%i,%i)",
	                  Config.ServerID, Config.ServerName, Config.CharIP, Config.CharPort,
	                  Config.MaxConnections, Config.ParentID)) {
		Log(MSG_WARNING,
		    "Error accessing to database, the other server will not connect to CharServer");
	}
	...
	for (UINT i = 0; i < ClanList.size(); i++) {
		...
		result = DB->QStore("SELECT id,char_name,clan_rank FROM characters WHERE clanid=%i order by clan_rank",
		                    Clan->id);
