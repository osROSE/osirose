	...
	*/
	...
	if (data->btIsAllied) {
		...
		for (UINT j = 0; j < map->MonsterList.size(); j++) {
			...
			int iDistance = (int)GServer->distance(other->Position->current,
...
AICOND(003) {
	...
	int moveDistance = (int)GServer->distance(entity->Position->current,
...
AICOND(004) {
	...
	entity->UpdatePosition();
	int distance = (int)GServer->distance(entity->Position->current,
...
AICOND(006) {
	...
	GETAICONDDATA(006);
	dword value = (dword)(((float)entity->Stats->HP / (float)entity->Stats->MaxHP) *
	                      100.0f);
...
AICOND(013) {
	...
	if (target == NULL) {
		Log(MSG_DEBUG, "Check Target (1)not found btCheckTarget %i ",
		    data->btCheckTarget);
		return AI_FAILURE;
	}
...
AICOND(018) {
	...
	int distance = (int)GServer->distance(caller->Position->current,
	                                      thisMonster->Position->current);
