...
bool CPlayer::loaddata() {
	MYSQL_ROW row;
	MYSQL_RES *result =
	    GServer->DB->QStore("SELECT `level`,`face`,`hairStyle`,`sex`,`classid`,`zuly`,`str`,`dex`,`_int`, `con`,`cha`,`sen`,`curHp`,`curMp`,`id`,`statp`,`skillp`,`exp`,`stamina`,`quickbar`,`basic_skills`,`askill`,`askilllvl`,`pskill`,`pskilllvl`,`respawnid`,`clanid`,`clan_rank`,`townid` FROM `characters` WHERE `char_name`='%s'",
	                        CharInfo->charname);
	...
	if (mysql_num_rows(result) != 1) {
		Log(MSG_WARNING, "Number of user with charname '%s' is %i", CharInfo->charname,
	...
	GServer->DB->QFree();
	result = GServer->DB->QStore("SELECT unionid,union1points,union2points,union3points,union4points,union5points,union6points,union7points,union8points,union9points,union10points FROM characters WHERE char_name='%s'",
	                             CharInfo->charname);
	...
	GServer->DB->QFree();
	result = GServer->DB->QStore("SELECT itemnum,itemtype,refine,durability,lifespan,slotnum,count,stats,socketed,appraised,gem FROM items WHERE owner=%i",
	                             CharInfo->charid);
	...
	while (row = mysql_fetch_row(result)) {
		if (!GServer->IsValidItem(atoi(row[1]), atoi(row[0])) || atoi(row[6]) == 0) {
			Log(MSG_WARNING,
			    "char %s have a invalid or empty item in inventory: %i-%i [%i], this item will be deleted",
			    CharInfo->charname, atoi(row[1]), atoi(row[0]), atoi(row[6]));
			continue;
		}
	...
	GServer->DB->QFree();
	result = GServer->DB->QStore("SELECT itemnum,itemtype,refine,durability,lifespan,slotnum,count,stats,socketed,appraised,gem FROM storage WHERE owner=%i",
	                             Session->userid);
	...
	while (row = mysql_fetch_row(result)) {
		if (!GServer->IsValidItem(atoi(row[1]), atoi(row[0])) || atoi(row[6]) == 0) {
			Log(MSG_WARNING,
			    "char %s have a invalid or empty item in storage: %i%i [%i], this item will be deleted",
			    CharInfo->charname, atoi(row[1]), atoi(row[0]), atoi(row[6]));
			continue;
		}
	...
	if (Clan->clanid != 0) {
		result = GServer->DB->QStore("SELECT `logo`,`back`,`name`,`cp`,`grade` FROM `list_clan` where `id`=%i",
		                             Clan->clanid);
		...
		if (mysql_num_rows(result) != 1) {
			Log(MSG_WARNING, "There are %i clan(s) with id %i", mysql_num_rows(result),
			    Clan->clanid);
			Clan->clanid = 0;
		} else {
			row = mysql_fetch_row(result);
	...
	result = GServer->DB->QStore("SELECT quests from list_quest where owner=%i",
	                             CharInfo->charid);
	...
	GServer->DB->QExecute("UPDATE accounts SET online=1 where id=%u",
	                      Session->userid);
...
void CPlayer::savedata() {
	...
	if (Session->userid != 0) {
		...
		GServer->DB->QExecute("UPDATE characters SET level=%i,face=%i,hairStyle=%i,sex=%i,classid=%i,zuly=%i,str=%i,dex=%i,_int=%i, con=%i,cha=%i,sen=%i,curHp=%i,curMp=%i,id=%i,statp=%i,skillp=%i,exp=%i,stamina=%i,quickbar='%s',basic_skills='%s',askill='%s',askilllvl='%s',pskill='%s',pskilllvl='%s',respawnid=%i,clanid=%i,clan_rank=%i,townid=%i WHERE char_name='%s'",
		                      Stats->Level, CharInfo->Face, CharInfo->Hair, CharInfo->Sex, CharInfo->Job,
		                      CharInfo->Zulies, Attr->Str, Attr->Dex, Attr->Int, Attr->Con, Attr->Cha,
		                      Attr->Sen, hp, Stats->MP, CharInfo->charid, CharInfo->StatPoints,
		                      CharInfo->SkillPoints,
		                      CharInfo->Exp, CharInfo->stamina, quick, basic, active, activelvl, pasive,
		                      pasivelvl, Position->respawn, Clan->clanid, Clan->clanrank, Position->saved,
		                      CharInfo->charname);
		GServer->DB->QExecute("UPDATE characters SET unionid=%i,union1points=%i,union2points=%i,union3points=%i,union4points=%i,union5points=%i,union6points=%i,union7points=%i,union8points=%i,union9points=%i,union10points=%i WHERE char_name='%s'",
		                      Union_s->unionvar[0], Union_s->unionvar[1], Union_s->unionvar[2],
		                      Union_s->unionvar[3], Union_s->unionvar[4], Union_s->unionvar[5],
		                      Union_s->unionvar[6], Union_s->unionvar[7], Union_s->unionvar[8],
		                      Union_s->unionvar[9], Union_s->unionvar[10], CharInfo->charname);

		if (!GServer->DB->QExecute("DELETE FROM items WHERE owner=%i",
		                           CharInfo->charid)) {
			return;
		}
		...
		for (UINT i = 0; i < MAX_INVENTORY; i++) {
			if (items[i].count > 0) {
				GServer->DB->QExecute("INSERT INTO items (owner,itemnum,itemtype,refine,durability,lifespan,slotnum,count,stats,socketed,appraised,gem) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i)",
				                      CharInfo->charid, items[i].itemnum, items[i].itemtype, items[i].refine,
				                      items[i].durability,
				                      items[i].lifespan, i, items[i].count, items[i].stats,
				                      (items[i].socketed ? 1 : 0),
				                      (items[i].appraised ? 1 : 0), items[i].gem);
			}
		}

		if (!GServer->DB->QExecute("DELETE FROM storage WHERE owner=%i",
		                           Session->userid)) {
			return;
		}
		...
		for (UINT i = 0; i < 160; i++) {
			if (storageitems[i].count > 0) {
				GServer->DB->QExecute("INSERT INTO storage (owner,itemnum,itemtype,refine,durability,lifespan,slotnum,count,stats,socketed,appraised,gem) VALUES(%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i)",
				                      Session->userid, storageitems[i].itemnum, storageitems[i].itemtype,
				                      storageitems[i].refine, storageitems[i].durability,
				                      storageitems[i].lifespan, i, storageitems[i].count, storageitems[i].stats,
				                      (storageitems[i].socketed ? 1 : 0),
				                      (storageitems[i].appraised ? 1 : 0),
				                      storageitems[i].gem); //Gem Fix by Irulagain
			}
		}

		savequests(this);
		GServer->DB->QExecute("UPDATE accounts SET zulystorage = %i WHERE id = %i",
		                      CharInfo->Storage_Zulies, Session->userid);
...
void CPlayer::savequests(CPlayer *thisclient) {
	...
	mysql_real_escape_string(GServer->DB->Mysql, questBuffer, (const char *)&quest,
	                         sizeof(SQuestData));
	GServer->DB->QExecute("DELETE FROM list_quest WHERE owner=%i",
