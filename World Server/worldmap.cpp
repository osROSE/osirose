...
CMonster *CMap::AddMonster(UINT montype, fPoint position, UINT owner,
                           CMDrops *MonsterDrop, CMDrops *MapDrop, UINT spawnid , bool GetDropData) {
	...
	CMonster *monster = new(nothrow) CMonster(position, montype, this->id, owner,
	        spawnid);
...
#ifndef USEIFO
		CSpawnArea *thisspawn = GServer->GetSpawnArea(monster->Position->respawn,
		                        monster->Position->Map);

		if (thisspawn != NULL) {
			if (thisspawn->amon >=
			        thisspawn->max) { // reset spawn timer if the spawn is full
...
#else
		CMobGroup *thisgroup = GServer->GetMobGroup(monster->Position->respawn,
		                       monster->Position->Map);

		if (thisgroup != NULL) {
			if (thisgroup->active >=
			        thisgroup->limit) { // reset spawn timer if the spawn is full
