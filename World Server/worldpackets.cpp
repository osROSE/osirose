...
void CWorldServer::pakPlayer(CPlayer *thisclient) {
	...
	for (char i = 0; i < 30; i++) {
		ADDWORD(pak, thisclient->askill[i] +
		        thisclient->askilllvl[i]);    //Active skills
	}

	for (char i = 0; i < 30; i++) {
		ADDWORD(pak, thisclient->pskill[i] +
		        thisclient->pskilllvl[i]);    //Passive skills
	}
...
void CWorldServer::pakQuestData(CPlayer *thisclient) {
	...
	for (dword i = 0; i < 5; i++) {
		pakout.AddWord(thisclient->quest.EpisodeVar[i]);
	}

	for (dword i = 0; i < 3; i++) {
		pakout.AddWord(thisclient->quest.JobVar[i]);
	}

	for (dword i = 0; i < 7; i++) {
		pakout.AddWord(thisclient->quest.PlanetVar[i]);
	}

	for (dword i = 0; i < 10; i++) {
		pakout.AddWord(thisclient->quest.UnionVar[i]);
	}
	...
	for (unsigned i = 0; i < 10; i++) {
		...
		if (thisclient->quest.quests[i].QuestID > 0
		        && STB_QUEST.rows[thisclient->quest.quests[i].QuestID][1] > 0) {
			Time += thisclient->quest.quests[i].StartTime; // Start time
			Time += STB_QUEST.rows[thisclient->quest.quests[i].QuestID][1] *
			        10; // Time to finish
		...
		for (dword j = 0; j < 10; j++) {
			pakout.AddWord(thisclient->quest.quests[i].Variables[j]);
		}

		for (dword j = 0; j < 4; j++) {
			pakout.AddByte(thisclient->quest.quests[i].Switches[j]);
		}
...
bool CWorldServer::pakDoIdentify(CPlayer *thisclient, CPacket *P) {
	...
	MYSQL_RES *result =
	    DB->QStore("SELECT username,lastchar,accesslevel,zulystorage FROM accounts WHERE id=%i AND password='%s'",
	               thisclient->Session->userid, thisclient->Session->password);
	...
	if (mysql_num_rows(result) != 1) {
		Log(MSG_HACK,
		    "Someone tried to connect to world server with an invalid account");
		DB->QFree();
		return false;
	}
	...
	Log(MSG_INFO, "User '%s'(#%i) logged in with character '%s'",
	    thisclient->Session->username, thisclient->Session->userid,
...
bool CWorldServer::pakDoID(CPlayer *thisclient, CPacket *P) {
	...
	if (thisclient->clientid <= 0) {
		Log(MSG_WARNING, "User '%s'(#%i) denied access. Server is full.",
		    thisclient->Session->username, thisclient->Session->userid);
		return false;
	}

	Log(MSG_INFO, "User '%s'(#%i) assigned id #%i", thisclient->Session->username,
	    thisclient->Session->userid, thisclient->clientid);
	...
	int weight = (thisclient->GetCurrentWeight() / thisclient->GetMaxWeight()) *
	             100;
...
bool CWorldServer::pakSpawnNPC(CPlayer *thisclient, CNPC *thisnpc) {
	...
	if (thisnpc->npctype == 1115) ADDBYTE(pak,
		                                      GServer->Config.Cfmode)   // Burland Clan Field open/close
		else {
			ADDBYTE(pak, 0);
		}
	...
	switch (stancenum) {
		...
		case 1:
			if (thisclient->Status->Stance == RUNNING
			        || thisclient->Status->Stance ==
			        WALKING) { //>Status->Stance!=DRIVING && thisclient->Status->Stance>0x01)
				newstancenum = 1;
				thisclient->Status->Stance = SITTING;
			} else if (thisclient->Status->Stance ==
			           SITTING) { //(thisclient->Status->Stance<0x04)
				newstancenum = 3;
				thisclient->Status->Stance = RUNNING;
			}
...
bool CWorldServer::pakDoDrop(CPlayer *thisclient, CPacket *P) {
	...
	if (itemid == 0) {
		...
		if (thisclient->items[itemid].itemtype >= 10
		        && thisclient->items[itemid].itemtype <= 13) {
...
bool CWorldServer::pakPickDrop(CPlayer *thisclient, CPacket *P) {
	...
	if (thisdrop->item.itemnum > 250 && thisdrop->item.itemnum < 272
	        && thisdrop->item.itemtype ==
	        7) { //&&thisdrop->item.itemtype<174)//251/261/271/socket jewel 171Ring 172Necklace 173Earrings
	...
	if ((thisdrop->owner == 0 || thisdrop->owner == thisclient->CharInfo->charid
	        || time(NULL) - thisdrop->droptime >= 30) || (thisclient->Party->party != NULL
	                && thisclient->Party->party == thisdrop->thisparty)) {
		if (thisclient->Party->party != NULL) {
			...
			if (dropparty == 8) {   // Orderly
				...
				if (thisdrop->type == 1) {
					...
					if (thisclient->Party->party->LastZulies >=
					        thisclient->Party->party->Members.size()) {
					...
					if (thisclient->Party->party->LastItem >=
					        thisclient->Party->party->Members.size()) {
		...
		if (thisdrop->type == 1) {   //Zulie
			...
			if (type == 320 && thisdrop->item.itemtype == 10) {
				...
				switch (UseList.Index[thisdrop->item.itemnum]->useeffect[0]) {
					...
					case 76:
						dropowner->CharInfo->stamina +=
						    (UseList.Index[thisdrop->item.itemnum]->useeffect[1]);
					...
					case 92:
						if (dropowner->Clan->clanid > 0) {
							...
							DB->QExecute("UPDATE list_clan set cp=%i where id=%i", dropowner->Clan->CP,
							             dropowner->Clan->clanid);
					...
					default:
						Log(MSG_WARNING, "Unknown autoconsume type: %i %i",
						    (UseList.Index[thisdrop->item.itemnum]->usecondition[0]),
						    (UseList.Index[thisdrop->item.itemnum]->usecondition[1]));
						break;
				}
			} else {
...
bool CWorldServer::pakAddStats(CPlayer *thisclient, CPacket *P) {
	...
	if (thisclient->CharInfo->StatPoints >= nstatval
	        && nstatval < ((Config.MaxStat / 5) + 1)) {
		switch (statid) {
			case 0:
...
bool CWorldServer::pakUserDied(CPlayer *thisclient, CPacket *P) {
	...
	if (respawn == 1 || map->dungeon) {
		...
		if (thisrespawn != NULL) {
			...
			MapList.Index[thisclient->Position->saved]->TeleportPlayer(thisclient, coord,
			        false);
		}
	}
...
bool CWorldServer::pakSaveTown(CPlayer *thisclient, CPacket *P) {
	...
	GServer->DB->QExecute("UPDATE characters SET townid=%i WHERE id=%i",
	                      thisclient->Position->saved, thisclient->CharInfo->charid);
	return true;
}
// Shouting
...
bool CWorldServer::pakStartSkill(CPlayer *thisclient, CPacket *P) {
	...
	if (skillnum >= MAX_SKILL) {
		Log(MSG_HACK, "Invalid Skill id %i for %s ", skillnum,
		    thisclient->CharInfo->charname);
		return false;
	}

	if (skillnum > 29) {
		skillid = thisclient->askill[skillnum - 30] + thisclient->askilllvl[skillnum -
		          30];
	} else {
...
bool CWorldServer::pakTradeAction(CPlayer *thisclient, CPacket *P) {
	...
	CPlayer *otherclient = GetClientByID(thisclient->Trade->trade_target,
			...
			if (otherclient->Trade->trade_status == 6) {
				...
				for (unsigned i = 0; i < 10; i++) {
					if (thisclient->Trade->trade_count[i] > 0) {
						...
						thisclient->items[thisclient->Trade->trade_itemid[i]].count -=
						    thisclient->Trade->trade_count[i];
						...
						ADDWORD(pakt, BuildItemHead(
						            thisclient->items[thisclient->Trade->trade_itemid[i]]));
						ADDDWORD(pakt, BuildItemData(
						             thisclient->items[thisclient->Trade->trade_itemid[i]]));
					...
					if (otherclient->Trade->trade_count[i] > 0) {
						...
						otherclient->items[otherclient->Trade->trade_itemid[i]].count -=
						    otherclient->Trade->trade_count[i];
						...
						ADDWORD(pako, BuildItemHead(
						            otherclient->items[otherclient->Trade->trade_itemid[i]]));
						ADDDWORD(pako, BuildItemData(
						             otherclient->items[otherclient->Trade->trade_itemid[i]]));
...
bool CWorldServer::pakSkillSelf(CPlayer *thisclient, CPacket *P) {
	...
	if (num >= MAX_SKILL) {
		Log(MSG_HACK, "Invalid Skill id %i for %s ", num,
		    thisclient->CharInfo->charname);
		return false;
	}
...
bool CWorldServer::pakUseItem(CPlayer *thisclient, CPacket *P) {
	...
	switch (thisuse->usescript) {
		...
		case 3: { // Charm Scroll
			...
			if (thisskill == NULL) {
				Log(MSG_WARNING, "Char %s tried to use invalid scroll type: %i",
				    thisclient->CharInfo->charname, thisuse->usetype);
				delete thisuse;
				return true;
			}
...
bool  CWorldServer::pakLevelUpSkill(CPlayer *thisclient, CPacket *P) {
	...
	if (pos > 59) { // Passive Skill - ID + 60
		thisskill = GetSkillByID((thisclient->pskill[pos - 60]) +
		                         (thisclient->pskilllvl[pos - 60]));
		nextskill = GetSkillByID(((thisclient->pskill[pos - 60]) +
		                          (thisclient->pskilllvl[pos - 60])) + 1);
	} else if (pos > 29) { // Active Skill - ID + 30
		thisskill = GetSkillByID((thisclient->askill[pos - 30]) +
		                         (thisclient->askilllvl[pos - 30]));
		nextskill = GetSkillByID(((thisclient->askill[pos - 30]) +
...
bool CWorldServer::pakCraft(CPlayer *thisclient, CPacket *P) {
	...
	if (changeofstatslow + int(changeofstatsrange * rand() /
	                           (RAND_MAX + 1.0)) > 50) {
		int statslowget = 1;
	...
	if (newslot != 0xffff) {
		...
		for (char used = 4; used != 12; used += 2) {
			...
			if (material != 0) {
				thisclient->items[material].count -=
				    ProductList.Index[materialnumber]->amount[m];
				m += 1;
		...
		int crafting_exp = item.durability + changeofstatsrange *
		                   (thisclient->Stats->Level / 15);
	...
	*/
	...
	switch (action) {
		case 0x00: { //Deposit
			...
			if (newitem.itemtype >= 10 && newitem.itemtype <= 12) { //is it a stackable type
				...
				if (thisclient->storageitems[newslot].count > 0) {
					thisclient->storageitems[newslot].count +=
					    newitem.count;    //add the new items to the storage slot
				} else {
					thisclient->storageitems[newslot] = newitem;
				}

				thisclient->items[itemslot].count -=
				    count; //delete the items from inventory slot
		...
		case 0x01: { //Withdraw
			...
			if (storageslot >= 160) {
				Log(MSG_HACK, "Invalid storage slot %i from %s", storageslot,
				    thisclient->Session->username);
				return false;
			}
...
bool CWorldServer::pakOpenShop(CPlayer *thisclient, CPacket *P) {
	...
	strncpy(thisclient->Shop->name , (char *) & (*P).Buffer[nchar] ,
	        P->Size - nchar);
	...
	for (int i = 0; i < nselling; i++) {
		...
		if (thisclient->items[slot].itemtype > 9
		        && thisclient->items[slot].itemtype < 14) {
...
bool CWorldServer::pakBuyShop(CPlayer *thisclient, CPacket *P) {
	...
	switch (action) {
		case 0x01: { //check this
			...
			if (thisclient->CharInfo->Zulies < (otherclient->Shop->SellingList[slot].price *
			                                    count)) {
				return true;
			}
			...
			thisclient->CharInfo->Zulies -= (otherclient->Shop->SellingList[slot].price *
			                                 count);
			otherclient->CharInfo->Zulies += (otherclient->Shop->SellingList[slot].price *
			                                  count);
...
bool CWorldServer::pakSellShop(CPlayer *thisclient, CPacket *P) {
	...
	switch (action) {
		case 0x01: {
			...
			if (otherclient->CharInfo->Zulies < (otherclient->Shop->BuyingList[slot].price *
			                                     count)) {
				return true;
			}
			...
			thisclient->CharInfo->Zulies += (otherclient->Shop->BuyingList[slot].price *
			                                 count);
			otherclient->CharInfo->Zulies -= (otherclient->Shop->BuyingList[slot].price *
			                                  count);
...
bool CWorldServer::pakModifiedItem(CPlayer *thisclient, CPacket *P) {
	...
	switch (action) {
		...
		case 0x03: { //itemdivide by npc
			...
			CItem splititem;
			char updateteller = 0;
			UINT material_lookup_number = STB_ITEM[(thisclient->items[slot].itemtype) -
			                                       1].rows[thisclient->items[slot].itemnum][14];

			for (unsigned int i = 0; i < STB_ITEM[11].rowcount; i++) {
				if (STB_ITEM[11].rows[i][4] ==
				        ProductList.Index[material_lookup_number]->item[0]) {
			...
			for (char teller = 1; teller < 4; teller++) {
				if (ProductList.Index[material_lookup_number]->amount[teller] > 0) {
					...
					splititem.itemtype = (ProductList.Index[material_lookup_number]->item[teller]) /
					                     1000;
					splititem.itemnum  = (ProductList.Index[material_lookup_number]->item[teller])
					                     - (1000 * splititem.itemtype);
					splititem.appraised = false;
					splititem.durability = 0;
					splititem.gem = 0;
					splititem.lifespan = 0;
		...
		case 0x05: { //Player Refine
			...
			if (thisclient->items[item].count < 1
...
bool CWorldServer::pakRepairHammer(CPlayer *thisclient, CPacket *P) {
	Log(MSG_INFO, "Repairhammer. %02x %02x %02x %02x", GETBYTE((*P), 0),
	    GETBYTE((*P), 1), GETBYTE((*P), 2), GETBYTE((*P), 3));
...
bool CWorldServer::pakShowHeal(CPlayer *thisclient, CPacket *P) {
	...
	CCharacter *character = MapList.Index[thisclient->Position->Map]->GetCharInMap(
	                            clientid);
