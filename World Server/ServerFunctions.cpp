...
bool CWorldServer::DoSkillScript(CCharacter *character, CSkills *thisskill) {
	try {
		...
		Log(MSG_WARNING,
		    "Error in CWorldServer::DoSkillScript( CCharacter* character, CSkills* thisskill )(serverFunctions.cpp)");
		return false;
	}
}
...
CDrop *CWorldServer::GetDrop(CMonster *thismon) {
	try {
		...
		if (randv <= 30) { //30% zuly [zulies will count as mapdrop]
			if (thismon->MonsterDrop->mapdrop->level_max >=
			        thismon->MonsterDrop->firstlevel) {
				newdrop->type = 1; //Drop Zuly
				newdrop->amount = thismon->thisnpc->level * 5 * Config.ZULY_RATE + RandNumber(1,
				                  10);
				return  newdrop;
			}
		...
		if (newdrop->item.itemtype > 1 && newdrop->item.itemtype < 10
		        && newdrop->item.itemtype != JEWEL) {
		...
		Log(MSG_WARNING,
		    "Error in CWorldServer::GetDrop( CMonster* thismon )(serverFunctions.cpp)");
		return NULL;
	}
}
...
CDrop *CWorldServer::GetPYDrop(CMonster *thismon, UINT droptype) {
	...
	float leveldif = (float)thismon->thisnpc->level - (float)
	                 thisclient->Stats->Level;
	float dropchance = (droprate + (droprate * 0.01 * leveldif));

	if (dropchance < 10) {
		dropchance =
		    10;    //always a small chance of a drop even when the mob is more than 20 levels beneath your own
	}
	...
	if (thismon->IsGhost()) {
		...
		if (selection <= 5) {
			newdrop->type = 1; //Drop Zuly.
			newdrop->amount = thismon->thisnpc->level * 10 * Config.ZULY_RATE + RandNumber(
			                      1, 10);
			return  newdrop;
		} else {
			for (int i = 0; i < SkillbookList.size(); i++) {
				...
				if (thisdrop->level_min <= thismon->thisnpc->level
				        &&  thisdrop->level_max >= thismon->thisnpc->level) {
		...
		for (int i = 0; i < MDropList.size(); i++) {
			...
			if (thisdrop->map == thismon->Position->Map && dropmode == 1) { // map drop
				...
				if (thismon->thisnpc->level == 1) {
					test = GServer->RandNumber(1,
					                           10000);    // make it less likely to get map drops from event mobs
				}
			...
			if (thismon->thisnpc->level >= thisdrop->level_min
			        && thismon->thisnpc->level <= thisdrop->level_max && dropmode == 3) {
			...
			if (isdrop == true) {
				if (droptype !=
				        1) { //side drops only. Skip if the item is not a match for side type
	...
	if (newdrop->item.itemtype == 8 || newdrop->item.itemtype == 9) {
		...
		if (psocked <
		        Config.SlotChance) { //default should be around 5% needs to be rare
	...
	int basedrop = 6 +
	               chamod; //Base number of items to be dropped. add CHA to increase this.
	...
	if (newdrop->item.itemtype == 10 || newdrop->item.itemtype == 12) {
		...
		if (thismon->thisnpc->level == 1 && newdrop->item.count > 6) {
			newdrop->item.count =
			    6;    //limit the drop rate of items from level 1 event mobs
		}
		...
		if (newdrop->item.itemtype == 10) {
			if (newdrop->item.itemnum >= 441
			        && newdrop->item.itemnum <= 888) { // skillbooks
				newdrop->item.count = 1;
			}
		}
		...
		if (newdrop->item.itemtype == 12) {
			if (newdrop->item.itemnum > 300
			        && newdrop->item.itemnum < 360) { //bullets get a lot higher count.
				newdrop->item.count *= 10;
				newdrop->item.count += 10;
			}
		}
	} else if (newdrop->item.itemtype > 1 && newdrop->item.itemtype != 7
	           && newdrop->item.itemtype < 10) {
		...
		if (bluechance1 <
		        bluechance2) { // some percentage of drops will be specials or blues whenever one is available.
...
void CWorldServer::DoFairyFree(int fairy) {
	...
	if (GServer->Config.FairyTestMode == 0) {
		GServer->FairyList.at(fairy)->WaitTime = GServer->Config.FairyWait *
		        (rand() % GServer->GetFairyRange(1) + GServer->GetFairyRange(0));
	}
...
void CWorldServer::RefreshFairy() {
	...
	if (Config.FairyMode == 1
	        && ClientList.size() > 1) {  //if fairy mode on and someone online
		for (int i = 0; i < Config.FairyMax; i++) { // check all fairies
			if (FairyList.at(i)->LastTime <= (clock() - (FairyList.at(
			                                      i)->WaitTime * 60 * CLOCKS_PER_SEC))
			        && !FairyList.at(i)->assigned) {   // if fairy hour is now
				if (ClientList.size() >=
				        5) { //stop solo players fromgetting fairied and screwing up their stats during testing.
					int value = rand() % (int)(ClientList.size() - 1) +
					            1; //choose random value in clientlist
					...
					targetclient->FairyListIndex =
					    i;                 // FairyList index number of our actual fairy
					DoFairyStuff(targetclient, 1);           // spawn fairy to target
					targetclient->SetStats();
				}
			}

			if ((FairyList.at(i)->LastTime + (Config.FairyStay * 60 * CLOCKS_PER_SEC)) <=
			        clock() && FairyList.at(i)->assigned) { // id time for our fairy to go away
				CPlayer *oldclient  = (CPlayer *) ClientList.at(FairyList.at(
				                          i)->ListIndex)->player;
				DoFairyFree(i);
	...
	if (Config.FairyMode == 0
	        && ClientList.size() > 1) {   // if serevr mode off and someone online
		for (int i = 0; i < Config.FairyMax; i++) {
			if ((FairyList.at(i)->LastTime + (Config.FairyStay * 60 * CLOCKS_PER_SEC)) <=
			        clock() && FairyList.at(i)->assigned) {
				CPlayer *oldclient  = (CPlayer *) ClientList.at(FairyList.at(
				                          i)->ListIndex)->player;
				DoFairyFree(i);
				oldclient->Fairy = 0;
