...
// -- CLIENT --
void CWorldServer::SendToVisible(CPacket *pak, CPlayer *thisclient,
                                 bool dothisclient) {
...
// -- MONSTER --
void CWorldServer::SendToVisible(CPacket *pak, CMonster *thismon,
                                 CDrop *thisdrop) {
...
// -- CHARACTER --
void CWorldServer::SendToVisible(CPacket *pak, CCharacter *character,
                                 CDrop *thisdrop) {
...
bool CWorldServer::DeleteSpawn(CSpawnArea *spawn) {
	...
	for (UINT i = 0; i < MapList.Index[spawn->map]->MonsterSpawnList.size(); i++) {
		if (MapList.Index[spawn->map]->MonsterSpawnList.at(i) == spawn) {
			MapList.Index[spawn->map]->MonsterSpawnList.erase(
			    MapList.Index[spawn->map]->MonsterSpawnList.begin() + i);
			delete spawn;
			return true;
		}
	}
...
float CWorldServer::AreaOfTriangle(fPoint p1, fPoint p2, fPoint p3) {
	return abs((int)((p2.x * p1.y - p1.x * p2.y) + (p3.x * p2.y - p2.x * p3.y) +
	                 (p1.x * p3.y - p2.x * p1.y))) / 2;
}
...
bool CWorldServer::IsMonInCircle(CPlayer *thisclient, CMonster *thismon,
                                 float radius) {
...
bool CWorldServer::IsPlayerInCircle(CPlayer *thisclient, CPlayer *otherclient,
                                    float radius) {
...
CUseInfo *CWorldServer::GetUseItemInfo(CPlayer *thisclient, unsigned int slot) {
	...
	switch (type) {
		...
		case 313: { //Magic Item [scrolls/summons/fireworks/etc]
			...
			if ((useitem->itemnum > 300 && useitem->itemnum < 320)
			        || (useitem->itemnum > 960 && useitem->itemnum < 965)) {
				useitem->usescript = 7;
				...
				if ((useitem->itemnum > 349 && useitem->itemnum < 355) ||
					...
					if ((useitem->itemnum > 380 && useitem->itemnum < 384)) {
						...
						if ((useitem->itemnum > 370 && useitem->itemnum < 377) ||
							...
							if (useitem->itemnum == 913 || useitem->itemnum == 918
							        || useitem->itemnum == 921 ||
...
bool CWorldServer::CheckInventorySlot(CPlayer *thisclient, int slot) {
	if (slot >= MAX_INVENTORY || slot < 0) {
		Log(MSG_HACK, "Invalid Inventory slot %i from %s", slot,
