...
public:
	...
	void ServerLoop();
	void LoadEncryption();
	bool pakGiveItem(CPlayer *thisclient, unsigned char itemtype,
	...
	//------------------ BUFFS (buff.cpp)
	CBValue GetBuffValue(CSkills *thisskill, CCharacter *character, UINT Evalue,
	                     UINT i, UINT up, UINT down, UINT CurrentValue, bool Buff = true,
	                     bool Status = false);
	...
	void SendToVisible(CPacket *pak, class CCharacter *character,
	                   CDrop *thisdrop = NULL);
	...
	bool pakGMItemtoplayer(CPlayer *thisclient, char *name , UINT itemid ,
	                       UINT itemtype , UINT itemamount , UINT itemrefine , UINT itemls,
	                       UINT itemstats , UINT itemsocket) ;
	bool pakGMItem(CPlayer *thisclient, UINT itemid , UINT itemtype ,
	               UINT itemamount , UINT itemrefine , UINT itemls, UINT itemstats ,
	               UINT itemsocket);
	bool pakGMTeleOtherPlayer(CPlayer *thisclient, char *name, int map, float x,
	                          float y);
	...
	clock_t
	LastUpdateTime;          // Store the last update World Time
	bool                   ServerOnline;
	string filename;
