...
unsigned int CPlayer::GetDodge() {
	...
	UINT Dodge = 0;
	Dodge = (UINT)floor((((Stats->Level * 0.3) + ((Attr->Dex + Attr->Edex) * 1.9)) +
	                     10) * 0.4);
	UINT ClothDodge = 0;
	...
	for (UINT i = 1; i < 12; i++) {
		if (items[i].count != 0) {
			...
			if (items[i].itemtype > 9) {
				Log(MSG_WARNING, "Char %s have equip invalid item: %i,%i", CharInfo->charname,
				    items[i].itemtype, items[i].itemnum);
				continue;
			}

			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[0] ==
			        A_DODGE) {
				Dodge += GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[1];
			}

			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[0] ==
			        A_DODGE) {
...
unsigned int CPlayer::GetAccury() {
	...
	if (items[7].count == 0) {
		...
		Accury = (UINT)floor(((Attr->Con + Attr->Econ + 10) * 0.8) + ((
		                         GServer->EquipList[WEAPON].Index[items[7].itemnum]->quality * 0.6) +
		                     (items[7].durability * 0.8)));
	}
...
unsigned int CPlayer::GetCritical() {
	UINT Critical = 0;
	Critical = (UINT)floor(((Attr->Con + Attr->Econ + 20) * 0.2) + Attr->Sen +
	                       Attr->Esen);
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats [from tomiz]
		if (items[i].count != 0) {
			...
			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[0] ==
			        A_CRITICAL) {
				Critical +=
				    GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[1];
			}

			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[0] ==
			        A_CRITICAL) {
				Critical +=
				    GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[1];
			}
...
unsigned int CPlayer::GetMagicDefense() {
	...
	MagicDefense += (UINT)floor(((Attr->Int + Attr->Eint + 5) * 0.6) + ((
	                                Stats->Level + 15) * 0.8));
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats [from tomiz]
		if (items[i].count != 0) {
			if (i < 9 && items[i].itemtype != WEAPON) {
				MagicDefense +=
				    GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->magicresistence;
			}
...
unsigned int CPlayer::GetAttackPower() {
	...
	if (items[7].itemnum != 0 && items[7].count > 0 && items[7].durability > 0) {
		...
		attack += (UINT)floor(((Attr->Dex + Attr->Edex) * 0.3) + ((
		                          Attr->Str + Attr->Estr) * 0.5) + (Stats->Level * 0.2));
	...
	switch (weapontype) {
		case ONE_HAND_SWORD: { //one hand sword
			attack += (UINT)floor((Attr->Str + Attr->Estr) * 0.75 + Stats->Level * 0.2 + ((
			                          Attr->Str + Attr->Estr) * 0.05 + 29) * weaponatk * 0.03333334);
		...
		case BOW: { //bow
			...
			attack += (UINT)floor(((Attr->Str + Attr->Estr + Stats->Level) * 0.2) + ((
			                          Attr->Dex + Attr->Edex) * 0.7) + ((Attr->Sen + Attr->Esen) * 0.0666666667) +
			                      (weaponatk * 1.2727) + arrows);
		...
		case DUAL_GUN: { //dual guns
			...
			attack += (UINT)floor((Attr->Con + Attr->Econ) * 0.47 + bullets + Stats->Level *
			                      0.1 + (Attr->Dex + Attr->Edex) * 0.3 + ((Attr->Con + Attr->Econ) * 0.04 +
			                              (Attr->Sen + Attr->Esen) * 0.05 + 29) * weaponatk * 0.03333334);
		...
		case LAUNCHER: { //launcher
			...
			attack += (UINT)floor(cannons * 0.8 + Stats->Level * 0.1 +
			                      (Attr->Str + Attr->Estr) * 0.32 + (Attr->Con + Attr->Econ) * 0.45 + ((
			                                  Attr->Con + Attr->Econ) * 0.04 + (Attr->Sen + Attr->Esen) * 0.05 + 29) *
			                      weaponatk * 0.03333334);
			skilltype = sGunAtkPow;
		}
		break;

		case STAFF: { //staff
			attack += (UINT)floor(Stats->Level * 0.2 + ((Attr->Int + Attr->Eint) +
			                      (Attr->Str + Attr->Estr)) * 0.4 + ((Attr->Int + Attr-> Eint) * 0.05 + 29) *
			                      weaponatk * 0.03333334);
		...
		case WAND: { //wand
			attack += (UINT)floor(Stats->Level * 0.2 + (Attr->Int + Attr->Eint) * 0.6 + (((
			                          Attr->Sen + Attr->Esen) * 0.1 + 26) * 0.03703704 * weaponatk));
			skilltype = sMagicAtkPow;
		}
		break;

		case KATAR: { //katar
			attack += (UINT)floor(Stats->Level * 0.2 + (Attr->Str + Attr->Estr) * 0.42 +
			                      (Attr->Dex + Attr->Edex) * 0.55 + ((Attr->Dex + Attr->Edex) * 0.05 + 20) *
			                      weaponatk * 0.0476190);
		...
		case DOUBLE_SWORD: { //double sword
			attack += (UINT)floor(Stats->Level * 0.2 + (Attr->Str + Attr->Estr) * 0.63 +
			                      (Attr->Dex + Attr->Edex) * 0.45 + ((Attr->Dex + Attr->Edex) * 0.05 + 25) *
			                      weaponatk * 0.03846154);
			skilltype = sCombatAtkPow;
		}
		break;
		...
		case CROSSBOW: { //crossbow
			...
			attack += (UINT)floor((((Attr->Str + Attr->Estr) + Stats->Level) * 0.1) + ((
			                          Attr->Dex + Attr->Edex) * 0.04 + (Attr->Sen + Attr->Esen) * 0.03 + 29) *
			                      weaponatk * 0.03333334 + (Attr->Dex + Attr->Edex) * 0.52 + arrows);
...
unsigned int CPlayer::GetDefense() {
	...
	defense += (UINT)floor(((Attr->Estr + Attr->Str + 5) * 0.35) + ((
	                           Stats->Level + 15) * 0.7));
	...
	for (UINT i = 1; i < 9; i++) {
		...
		if (items[i].count != 0) {
			...
			defense +=
			    GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->defense;

			if (items[i].refine > 0) {
				UINT refine = (UINT)floor((double)(items[i].refine / 16));

				if (refine < 10) {
					defense += (UINT)floor(extra_refine[refine] * 0.01 *
					                       GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->defense);
				}
			}
		}
	...
	for (UINT i = 0; i < 30; i++) {
		...
		for (UINT j = 0; j < 2; j++) {
			...
			for (int n = 0; n < 5; n++) {
				if (skill->weapon[n] == items[8].type
				        && items[8].count > 0) { // item defined in the skill is currently equipped in the shield slot
					if (skill->value2[j] > 0) {
						Extra += defense * skill->value2[j] / 100;
					}
...
unsigned int CPlayer::GetAttackSpeed() {
	...
	for (UINT i = 0; i < 30; i++) {
		...
		if (skill->weapon[0] == 0) {
			...
			for (int n = 0; n < 5; n++) {
				if (skill->weapon[n] != 0
				        && skill->weapon[n] == weapontype) { // item defined in the skill
					GotWeapon = true;
				}
			}
		}
		...
		if (GotWeapon) { //No point checking if we have the RIGHT weapon when we don't have any of them
			for (UINT j = 0; j < 2; j++) {
				...
				if (GotRightWeapon) {
					...
					if (this->GetStatDebug(11)) { //Debug code
						GServer->SendPM(this, "[AS] Added %i for weapon type %i. Skill ID %i", Extra,
						                weapontype, skill->id);
					}
				}
			}
...
unsigned int CPlayer::GetMoveSpeed() {
	...
	switch (Status->Stance) {
		...
		case RUNNING: {
			...
			for (UINT i = 0; i < 12; i++) { //cloth stats [from tomiz]
				...
				if (items[i].stats > 0 && items[i].stats < 500) {
					...
					if (this->GetStatDebug(6)) {
						GServer->SendPM(this, "[MS] %i - Added extra/gem stats, item slot %i", mspeed,
						                i);
					}
				}
			}
			...
			if (Status->Dash_up != 0xff) {
				...
				if (this->GetStatDebug(6)) { //Debug code
					GServer->SendPM(this, "[MS] %i - Speed buff(%i)",
					                MagicStatus[Status->Dash_up].Value, mspeed);
				}
			}
			...
			if (Status->Dash_down != 0xff) {
				...
				if (this->GetStatDebug(6)) { //Debug code
					GServer->SendPM(this, "[MS] %i - Speed DE-buff(%i)",
					                MagicStatus[Status->Dash_down].Value, mspeed);
				}
			}
		}
		break;
		...
		case DRIVING: { //cart or gear
			...
			if (this->items[135].itemnum > 30
			        && this->items[135].itemnum < 40) { //Castle gear body
				m_speed = 1000;
				...
				if (GServer->PatList.Index[items[138].itemnum]->condition[1] ==
				        23) { // accessory
...
unsigned int CPlayer::GetMaxHP() {
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats [from tomiz]
		if (items[i].count != 0) {
			...
			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[0] ==
			        A_MAX_HP ||
			        GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[0] ==
			        A_HP) {
			...
			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[0] ==
			        A_MAX_HP ||
			        GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[0] ==
			        A_HP) {
				hpmax += GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[1];
			}

			if (items[i].stats > 0 && items[i].stats < 500) {
				if (GServer->StatsList[items[i].stats].stat[0] == A_MAX_HP
				        || GServer->StatsList[items[i].stats].stat[0] == A_HP) {
					hpmax += GServer->StatsList[items[i].stats].value[0];
				}

				if (GServer->StatsList[items[i].stats].stat[1] == A_MAX_HP
				        || GServer->StatsList[items[i].stats].stat[0] == A_HP) {
...
unsigned int CPlayer::GetMaxMP() {
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats [from tomiz]
		if (items[i].count != 0) {
			...
			if (items[i].stats > 0 && items[i].stats < 500) {
				if (GServer->StatsList[items[i].stats].stat[0] == A_MAX_MP
				        || GServer->StatsList[items[i].stats].stat[0] == A_MP) {
					maxmp += GServer->StatsList[items[i].stats].value[0];
				}

				if (GServer->StatsList[items[i].stats].stat[1] == A_MAX_MP
				        || GServer->StatsList[items[i].stats].stat[0] == A_MP) {
...
unsigned int CPlayer::GetHPRegenAmount() {
	...
	for (UINT i = 0; i < 30; i++) {
		...
		for (UINT j = 0; j < 2; j++) {
			if (skill->buff[j] == sHPRate
			        || skill->buff[j] ==  sHPRecovery) { // Not sure on sHPRate
				if (skill->value2[j] > 0) {
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats [from tomiz]
		if (items[i].count != 0) {
			...
			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[0] ==
			        A_MP_REC_RATE) {
				amount += amount *
				          GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat1[1] / 100;
			}

			if (GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[0] ==
			        A_MP_REC_RATE) {
				amount += amount *
				          GServer->EquipList[items[i].itemtype].Index[items[i].itemnum]->stat2[1] / 100;
			}
	...
	for (UINT i = 1; i < 12; i++) { //cloth stats
		if (items[i].count > 0) {
			...
			if (items[i].gem != 0) { // jem stats
				UINT Stat1 = GServer->JemList.Index[items[i].gem] != NULL ?
				             GServer->JemList.Index[items[i].gem]->stat1[0] : 0;
				UINT Stat2 = GServer->JemList.Index[items[i].gem] != NULL ?
				             GServer->JemList.Index[items[i].gem]->stat2[0] : 0;
...
float CPlayer::GetAttackDistance() {
	...
	AttackDistance =
	    GServer->EquipList[WEAPON].Index[items[7].itemnum]->attackdistance / 100;
