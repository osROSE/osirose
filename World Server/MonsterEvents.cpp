...
bool CMonster::OnBeAttacked(CCharacter *Enemy) {
	if (thisnpc->aggresive ==
	        0) { //prevent monsters with zero aggresion fighting back.
		return true;
	}
...
bool CMonster::SummonUpdate(CMonster *monster, CMap *map, UINT j) {
	...
	if (etime >= 4 * CLOCKS_PER_SEC && Stats->HP > 0) {
		...
		if (Stats->HP < 1 || ownerclient == NULL
		        || ownerclient->IsDead()) { //orphan check
			...
			for (UINT i = 0; i < GServer->MapList.Index[Position->Map]->PlayerList.size();
			        i++) {
				CPlayer *thisclient = GServer->MapList.Index[Position->Map]->PlayerList.at(i);
				float tempdist = GServer->distance(Position->current,
				                                   thisclient->Position->current);

				if (tempdist < distance) {
					thisclient->Stats->HP += ((((montype - 800) * 5) + ((ownerclient->Stats->Level)
					                           / 10)) + 10);
					thisclient->Stats->MP += (((montype - 800) * 5) + ((ownerclient->Stats->Level) /
					                          25));
...
void CMonster::DoAi(int ainumber,
                    char type) { //ainumber is monster->AI type is add=0 idle=1 attacking=2 attacked=3 after killing target=4 hp<1=5
	CAip *script = NULL;
	...
	for (unsigned j = 0; j < GServer->AipList.size(); j++) {
		if (GServer->AipList.at(j)->AipID == aiindex) {
			...
			for (dword i = 0; i < script->ConditionCount; i++) {
				...
				success = (*GServer->aiCondFunc[command])(GServer, this,
				          script->Conditions[i]->data);
			...
			if (success == AI_SUCCESS) {
				for (dword i = 0; i < script->ActionCount; i++) {
					...
					success = (*GServer->aiActFunc[command])(GServer, this,
					          script->Actions[i]->data);
