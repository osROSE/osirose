...
bool CWorldServer::pakGMCommand(CPlayer *thisclient, CPacket *P) {
	...
	if (strcmp(command, "here") == 0) {
		...
		return pakGMTele(thisclient, thisclient->Position->Map,
		                 thisclient->Position->current.x, thisclient->Position->current.y);
	} else if (strcmp(command,
	                  "tele") == 0) { // **** TELEPORT TO MAX AND X Y POINTS *****
		...
		Log(MSG_GMACTION, " %s : /tele %i,%i,%i" , thisclient->CharInfo->charname, map,
		    x, y);
		...
		int min = atoi(tmp);
		Log(MSG_GMACTION, " %s : /mute %s %i" , thisclient->CharInfo->charname, id,
		    min);
		...
		Log(MSG_GMACTION, " %s : /ann %s" , thisclient->CharInfo->charname,
		    &P->Buffer[5]);
	...
	/*  else if(strcmp(command, "headsize")==0)
		...
		DB->QExecute("UPDATE characters SET face=%i WHERE id=%i",
		             thisclient->CharInfo->Face, thisclient->CharInfo->charid);
		...
		ADDWORD(pak, 0x40b3);
		thisclient->client->SendPacket(&pak);
		DB->QExecute("UPDATE characters SET hairStyle=%i WHERE id=%i",
		             thisclient->CharInfo->Hair, thisclient->CharInfo->charid);
		...
		sprintf(line0,
		        "There are currently %i players and %i game moderators currently connected.",
		        playercount, gmcount);
		...
		if (loc == 1) {
			...
			SendPM(thisclient,
			       "Please input a number after the go command, below is a list of places and their appropriate number");
	...
	//******************************* START RESPAWN ***************************
		...
		if (thisclient->GMRespawnPoints.b == 1 && thisclient->GMRespawnPoints.n < 50) {
			...
			thisclient->GMRespawnPoints.points[n].x = floor(
			            thisclient->Position->destiny.x);
			thisclient->GMRespawnPoints.points[n].y = floor(
			            thisclient->Position->destiny.y);
		...
		if (thisclient->GMRespawnPoints.n > 3 && thisclient->GMRespawnPoints.b == 1) {
			...
			for (int i = 0; i < n; i++) {
				sprintf(&points[strlen(points)], "|%.0f,%.0f",
				        thisclient->GMRespawnPoints.points[i].x,
				        thisclient->GMRespawnPoints.points[i].y);
			}

			DB->QExecute("INSERT into list_spawnareas (id,map,montype,respawntime,points,min,max,agressive) values (%i,%i,%i,%i,'%s',%i,%i,%i)",
			             id, thisclient->GMRespawnPoints.map, thisclient->GMRespawnPoints.mobID,
			             thisclient->GMRespawnPoints.respawntime,
			             points, thisclient->GMRespawnPoints.min, thisclient->GMRespawnPoints.max,
			             agressive);
			thisclient->GMRespawnPoints.b = 0;
		...
		Log(MSG_GMACTION, " %s : /level %i %s" , thisclient->CharInfo->charname, level,
		    name);
		...
		Log(MSG_GMACTION, " %s : /exp %i %s" , thisclient->CharInfo->charname, exp,
		    name);
		...
		Log(MSG_GMACTION, " %s : /mon %i,%i" , thisclient->CharInfo->charname, montype,
		    moncount);
		...
		Log(MSG_GMACTION, " %s : /learnskill %i", thisclient->CharInfo->charname,
		    skillId);
		...
		return true;
	                  ...
	                  "item") == 0) { //Modified by Hiei (added refine/socket/stats)
		...
		Log(MSG_GMACTION, " %s : /item %i,%i,%i,%i,%i,%i" ,
		    thisclient->CharInfo->charname, itemid, itemtype, itemamount , itemrefine ,
		    itemsocket , itemstats);
		return pakGMItem(thisclient , itemid , itemtype , itemamount , itemrefine ,
		                 itemls, itemstats , itemsocket);
		...
		Log(MSG_GMACTION, " %s : /drop %i,%i" , thisclient->CharInfo->charname,
		    itemtype, itemid);
		...
		int zuly = atoi(tmp);
		Log(MSG_GMACTION, " %s : /givezuly %s, %i" , thisclient->CharInfo->charname,
		    name, zuly);
		...
		Log(MSG_GMACTION, " %s : /npc %i, %i" , thisclient->CharInfo->charname, npcid,
		    npcdialog);
		...
		Log(MSG_GMACTION, " %s : /givefairy %s, %i" , thisclient->CharInfo->charname,
		    name, mode);
		...
		unsigned mode = atoi(tmp);
		Log(MSG_GMACTION, " %s : /fairymode mode: %i" , thisclient->CharInfo->charname,
		    mode);
		...
		Log(MSG_GMACTION, " %s : /fairywait value: %i" , thisclient->CharInfo->charname,
		    value);
		...
		unsigned value = atoi(tmp);
		Log(MSG_GMACTION, " %s : /fairystay value: %i" , thisclient->CharInfo->charname,
		    value);
		...
		unsigned mode = atoi(tmp);
		Log(MSG_GMACTION, " %s : /fairytestmode mode: %i" ,
		    thisclient->CharInfo->charname, mode);
		...
		float y = (float)atoi(tmp);
		Log(MSG_GMACTION, " %s : /move %s,%i,%i,%i" , thisclient->CharInfo->charname,
		    name, map, x, y);
		...
		Log(MSG_GMACTION, " %s : /give2 %s,%i,%i,%i,%i,%i,%i,%i" ,
		    thisclient->CharInfo->charname, name, itemid , itemtype , itemamount ,
		    itemrefine , itemls, itemstats , itemsocket);
		...
		char text[200];
		sprintf(text,
		        "Server will shutdown in %i minutes, Please logout NOW to be sure your information is saved correctly.",
		        minutes);
		BEGINPACKET(pak, 0x702);
		...
		ADDBYTE(pak, 0x00);
		SendToAll(&pak);
		pthread_create(&WorldThread[SHUTDOWN_THREAD], NULL,
		               (void *(__cdecl *)(void *))ShutdownServer, (PVOID)minutes);
		Log(MSG_GMACTION, " %s : /shutdown %u" , thisclient->CharInfo->charname,
		    shutdown);
		...
		for (int i = 0; i < 10; i++) {
			...
			SendPM(thisclient, "Quest[%i]: %i[%i] %i[%i] %i[%i] %i[%i] %i[%i]" ,
			       thisclient->quest.quests[i].QuestID,
			       thisclient->quest.quests[i].Items[0].itemnum,
			       thisclient->quest.quests[i].Items[0].count,
			       thisclient->quest.quests[i].Items[1].itemnum,
			       thisclient->quest.quests[i].Items[1].count,
			       thisclient->quest.quests[i].Items[2].itemnum,
			       thisclient->quest.quests[i].Items[2].count,
			       thisclient->quest.quests[i].Items[3].itemnum,
			       thisclient->quest.quests[i].Items[3].count,
			       thisclient->quest.quests[i].Items[4].itemnum,
			       thisclient->quest.quests[i].Items[4].count);
		}
		...
		Log(MSG_GMACTION, " %s : /shoptype %i" , thisclient->CharInfo->charname,
		    shoptype);
		...
		Log(MSG_GMACTION, " %s : /stat %s,%i" , thisclient->CharInfo->charname,
		    statname, statvalue);
		return pakGMStat(thisclient , statname , statvalue);
	} else if (strcmp(command,
	                  "FLY_MODE") == 0) { /// just trying some stuff this isn't working(yet?)
		char update[20];
		...
		ADDWORD(pak, 0x40b3);
		otherclient->client->SendPacket(&pak);
		Log(MSG_GMACTION, " %s : /stats %i %s", thisclient->CharInfo->charname,
		    statvalue, name);
		return true;
		...
		ADDWORD(pak, 0x40b3);
		otherclient->client->SendPacket(&pak);
		Log(MSG_GMACTION, " %s : /skillp %i %s", thisclient->CharInfo->charname, skillp,
		    name);
		return true;
		...
		Log(MSG_GMACTION, " %s : /killinrange %i" , thisclient->CharInfo->charname,
		    range);
		...
		CMonster *thismon = GetMonsterByID(thisclient->Battle->target,
		                                   thisclient->Position->Map);

		if (thismon == NULL) {
			CNPC *thisnpc = GetNPCByID(thisclient->Battle->target,
			                           thisclient->Position->Map);
		...
		if ((thisclient->CharInfo->LastGlobal + Config.Command_GlobalTime) <= seconds
		        or thisclient->Session->accesslevel > 100) {
			thisclient->CharInfo->LastGlobal = time(NULL);
			char outputmsg[200];
			sprintf(outputmsg, "%s %s", Config.Command_GlobalPrefix, &P->Buffer[3]);
			Log(MSG_INFO, "%s> %s %s", thisclient->CharInfo->charname,
			    Config.Command_GlobalPrefix, &P->Buffer[3]);
			SendGlobalMSG(thisclient, outputmsg);
		} else {
			long int remaining = (Config.Command_GlobalTime - (seconds -
			                      thisclient->CharInfo->LastGlobal));
			char buffer2[200];
			sprintf(buffer2,
			        "Please wait %i seconds before sending another global message.", remaining);
			SendPM(thisclient, buffer2);
		}
		...
		while (count <= (ClientList.size() - 1)) {
			...
			if (whoclient->Session->accesslevel > 100) {
				sprintf(line0, "%s - GM[%i]", whoclient->CharInfo->charname,
				        whoclient->Session->accesslevel);
			} else {
		...
		sprintf(line0, "There are currently %i players connected!",
		        ((ClientList.size() - 1) - hiddenam));
		...
		if (MapList.Index[thisclient->Position->Map] != NULL) {
			switch (time) {
				case MORNING:
					MapList.Index[thisclient->Position->Map]->MapTime =
					    MapList.Index[thisclient->Position->Map]->morningtime;
					SendSysMsg(thisclient, "Time changed!, rejoin to see change [MORNING]");
					break;

				case DAY:
					MapList.Index[thisclient->Position->Map]->MapTime =
					    MapList.Index[thisclient->Position->Map]->daytime;
					SendSysMsg(thisclient, "Time changed!, rejoin to see change [DAY]");
					break;

				case EVENING:
					MapList.Index[thisclient->Position->Map]->MapTime =
					    MapList.Index[thisclient->Position->Map]->eveningtime;
					SendSysMsg(thisclient, "Time changed!, rejoin to see change [EVENING]");
					break;

				case NIGHT:
					MapList.Index[thisclient->Position->Map]->MapTime =
					    MapList.Index[thisclient->Position->Map]->nighttime;
		...
		Log(MSG_GMACTION, " Rate for Player Dmg is now set at %i by %s" , rate,
		...
		unsigned rate = atoi(tmp);
		Log(MSG_GMACTION, " Rate for Monster Dmg is now set at %i by %s" , rate,
		    thisclient->CharInfo->charname);
		...
		unsigned mode = atoi(tmp);
		Log(MSG_GMACTION, " Clan field mode changed to %i by %s" , mode,
		    thisclient->CharInfo->charname);
		...
		Log(MSG_WARNING, "Invalid GM Command '%s' by '%s'", command,
		    thisclient->CharInfo->charname);
	}
...
bool CWorldServer::pakGMItem(CPlayer *thisclient, UINT itemid, UINT itemtype,
                             UINT itemamount, UINT itemrefine, UINT itemls, UINT itemstats,
                             UINT itemsocket) {
	CItem item;
	...
	if (newslot != 0xffff) {
		...
		char buffer[200];
		sprintf(buffer, "Item added! (ID: %i) (Type: %i) (Refine: %i) (Socket: %i)",
		        item.itemnum, item.itemtype, item.refine, item.socketed);
		BEGINPACKET(pak, 0x702);
...
bool CWorldServer::pakGMBan(CPlayer *thisclient, char *name) {
	...
	DB->QExecute("UPDATE accounts SET accesslevel='0' WHERE id=%i",
	             otherclient->Session->userid);
	BEGINPACKET(pak, 0x702);
...
bool CWorldServer::pakGMZuly(CPlayer *thisclient, int mode, int amount,
                             char *charname) {
...
bool CWorldServer::pakGMLevel(CPlayer *thisclient, int level, char *name) {
	...
	if (level < 0) {
		...
		for (int s = (otherclient->Stats->Level - level + 1);
		        s <= otherclient->Stats->Level; s++) {
			otherclient->CharInfo->StatPoints += int((s * 0.8) + 10);
			otherclient->CharInfo->StatPoints += (s + 2) / 2;
		}
	}
...
bool CWorldServer::ReloadMobSpawn(CPlayer *thisclient, int id) {
	...
	for (int j = 0; j < thisspawn->max; j++) {
		...
		map->AddMonster(thisspawn->montype, position, 0, thisspawn->mobdrop,
		                thisspawn->mapdrop, thisspawn->id, true);
	}
...
bool CWorldServer::pakGMTeleOtherPlayer(CPlayer *thisclient, char *name,
                                        int map, float x, float y) {
	CPlayer *otherclient = GetClientByCharName(name);
...
bool CWorldServer::pakGMTeleToPlayer(CPlayer *thisclient, char *name) {
	...
	if (otherclient != NULL) {
		MapList.Index[otherclient->Position->Map]->TeleportPlayer(thisclient,
		        otherclient->Position->current, false);
	} else {
		SendSysMsg(thisclient, "User does not exist or is not online!");
	}
...
bool CWorldServer::pakGMInfo(CPlayer *thisclient, char *name) {
	...
	if (otherclient != 0) {
		...
		thisclient->client->SendPacket(&pak);
		RESETPACKET(pak, 0x702);
		sprintf(buffer, "Account: %s | ID: %i" , otherclient->Session->username,
		        otherclient->Session->userid);
		ADDSTRING(pak, buffer);
		ADDBYTE(pak, 0);
		thisclient->client->SendPacket(&pak);
		RESETPACKET(pak, 0x702);
		sprintf(buffer, "Level: %i | Job: %s" , (otherclient->Stats->Level - 0) ,
		        jobname);
		ADDSTRING(pak, buffer);
		ADDBYTE(pak, 0);
		thisclient->client->SendPacket(&pak);
		RESETPACKET(pak, 0x702);
		sprintf(buffer, "Map: %i , X: %i , Y: %i", (otherclient->Position->Map - 0),
		        (int)otherclient->Position->current.x, (int)otherclient->Position->current.y);
		ADDSTRING(pak, buffer);
		ADDBYTE(pak, 0);
		thisclient->client->SendPacket(&pak);
	} else {
...
bool CWorldServer::pakGMFairyto(CPlayer *thisclient, char *name, int mode) {
	...
	if (!otherclient->Fairy && mode == 1) {
		...
		otherclient->SetStats();
		Log(MSG_INFO, "HP: %i  MP: %i  ATK: %i   DEF: %i   CRI: %i  MSPD: %i",
		    otherclient->Stats->MaxHP, otherclient->Stats->MaxMP,
		    otherclient->Stats->Attack_Power, otherclient->Stats->Defense,
		    otherclient->Stats->Critical, otherclient->Stats->Move_Speed);
	}

	if (otherclient->Fairy && mode == 0) {
		GServer->DoFairyFree(otherclient->FairyListIndex);
		GServer->FairyList.at(otherclient->FairyListIndex)->WaitTime =
		    GServer->Config.FairyWait;
...
// Give Item to Player
bool CWorldServer::pakGMItemtoplayer(CPlayer *thisclient, char *name ,
                                     UINT itemid, UINT itemtype, UINT itemamount, UINT itemrefine, UINT itemls,
                                     UINT itemstats, UINT itemsocket) {
	CItem item;
	item.count            = itemamount;
	item.durability        = 40;
	item.itemnum        = itemid;
...
bool CWorldServer::pakGMStat(CPlayer *thisclient, char *statname,
                             int statvalue) {
	int statid;
...
bool CWorldServer::pakGMServerInfo(CPlayer *thisclient) {
	...
	ADDBYTE(pak, 0);
	thisclient->client->SendPacket(&pak);
	sprintf(buffer, "Online Players: %i",
	        (int)ClientList.size() - 1);// -1 (we don't count charserver)
	...
	sprintf(buffer, "Exp %i | Zulies %i | Drops %i", Config.EXP_RATE,
	        Config.ZULY_RATE, Config.DROP_RATE);
	RESETPACKET(pak, 0x0784);
	ADDSTRING(pak, "[SYS]ServerInfo");
	ADDBYTE(pak, 0);
	ADDSTRING(pak, buffer);
	...
	if (MapList.Index[thisclient->Position->Map] != NULL) {
		...
		switch (MapList.Index[thisclient->Position->Map]->CurrentTime) {
			case MORNING:
				sprintf(buffer, "The Time is: Morning[%i]",
				        MapList.Index[thisclient->Position->Map]->MapTime %
				        MapList.Index[thisclient->Position->Map]->dayperiod);
				ADDSTRING(pak, buffer);
				break;

			case DAY:
				sprintf(buffer, "The Time is: Day[%i]",
				        MapList.Index[thisclient->Position->Map]->MapTime %
				        MapList.Index[thisclient->Position->Map]->dayperiod);
				ADDSTRING(pak, buffer);
				break;
			...
			case EVENING:
				sprintf(buffer, "The Time is: Evening[%i]",
				        MapList.Index[thisclient->Position->Map]->MapTime %
				        MapList.Index[thisclient->Position->Map]->dayperiod);
				ADDSTRING(pak, buffer);
				break;
			...
			case NIGHT:
				sprintf(buffer, "The Time is: Night[%i]",
				        MapList.Index[thisclient->Position->Map]->MapTime %
				        MapList.Index[thisclient->Position->Map]->dayperiod);
				ADDSTRING(pak, buffer);
				break;
			...
			default:
				sprintf(buffer, "Invalid Time is the End of world [%i]",
				        MapList.Index[thisclient->Position->Map]->MapTime);
				ADDSTRING(pak, buffer);
		}
...
bool CWorldServer::GMShowTargetInfo(CPlayer *thisclient) {
	...
	char buffer[200];
	CMonster *monster = GetMonsterByID(thisclient->Battle->target,
	                                   thisclient->Position->Map);
	...
	float dist = distance(thisclient->Position->current,
	                      monster->Position->current);
	sprintf(buffer, "Target Position: %.4f, %.4f", monster->Position->current.x,
	        monster->Position->current.y);
	...
	ADDBYTE(pak, 0);
	thisclient->client->SendPacket(&pak);
	sprintf(buffer, "Target HP/MAXHP: %i/%i", monster->Stats->HP,
	        monster->Stats->MaxHP);
	RESETPACKET(pak, 0x784);
	ADDSTRING(pak, "[SYS]TargetInfo");
	ADDBYTE(pak, 0);
...
bool CWorldServer::pakGMKillInRange(CPlayer *thisclient, int range) {
	for (UINT j = 0;
	        j < MapList.Index[thisclient->Position->Map]->MonsterList.size(); j++) {
...
bool CWorldServer::pakGMClass(CPlayer *thisclient, char *classid) {
	...
	if (strcmp(classid , "Visitor") == 0 || strcmp(classid , "visitor") == 0) {
		...
		classid_new = 411;
	           ...
	           || strcmp(classid , "bourgeois") == 0) {
		classid_new = 421;
	} else if (strcmp(classid , "Artisan") == 0
	           || strcmp(classid , "artisan") == 0) {
		classid_new = 422;
	} else {
		return true;
	}
...
bool CWorldServer::pakGMMoveTo(CPlayer *thisclient, fPoint position) {
	if (thisclient->Shop->open || (!thisclient->Ride->Drive
	                               && thisclient->Ride->Ride) || !thisclient->Status->CanMove) {
		return true;
	}
