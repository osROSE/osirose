...
// Add Player buffs
bool CWorldServer::AddBuffs(CSkills *thisskill, CCharacter *character,
                            int Evalue) {
	bool bflag = false;
...
// Add Player DeBuffs
bool CWorldServer::AddDeBuffs(CSkills *thisskill, CCharacter *character,
                              int Evalue) {
	bool bflag = false;
...
// Add DeBuffs
bool CWorldServer::CheckABuffs(CSkills *thisskill, CCharacter *character,
                               int Evalue , int i) {
...
// Add Buffs
bool CWorldServer::CheckDBuffs(CSkills *thisskill, CCharacter *character,
                               int Evalue , int i) {
	...
	switch (thisskill->buff[i]) {
		...
		case A_STUN: {
			...
			if (BuffValue.NewValue != 0 && (tmp < (Evalue + 200) / 7)) {
				...
				printf("stunning failed: BuffValue.NewValue= %i, rand= %i, (Evalue+200)/700= %i \n",
		...
		case A_POISON: {
			...
			if (BuffValue.NewValue != 0
			        && (RandNumber(1, 100) < thisskill->success) /*(Evalue+200)/700) */) {
		...
		case A_MUTE: {
			...
			if (BuffValue.NewValue != 0 && (tmp < (Evalue + 200) / 7)) {
				...
				printf("muting failed: BuffValue.NewValue= %i, rand= %i, (Evalue+200)/700= %i \n",
				       BuffValue.NewValue, tmp, (Evalue + 200) / 700);
			}
		}
		break;
...
CBValue CWorldServer::GetBuffValue(CSkills *thisskill, CCharacter *character,
                                   UINT Evalue, UINT i, UINT up, UINT down, UINT CurrentValue, bool Buff,
                                   bool Status) {
	CBValue NewValue;
...
unsigned int CWorldServer::BuildBuffs(CCharacter *character) {
	...
	return (buff1 * 0x01) + (buff2 * 0x100) + (buff3 * 0x10000) +
	       (buff4 * 0x1000000);
}
// Build Buffs to Show
