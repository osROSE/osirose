...
public:
	...
	*/
	CFILE_UINT Read(void *buf, CFILE_SIZE element_size,
	                CFILE_SIZE element_count = 1) {
		if (fh == NULL) {
			DoError(FE_NOFILEOPENED);
			return 0;
		}
	...
	/** Reads from the file starting at [offset] relative to [origin]
	    ...
	    Unsigned int indicating the number of bytes read
	*/
	CFILE_UINT ReadFrom(void *buf, CFILE_UINT element_size,
	                    CFILE_UINT element_count, CFILE_SLONG offset, signed int origin = SEEK_SET) {
		if (fh == NULL) {
			DoError(FE_NOFILEOPENED);
			return 0;
		}
	...
	*/
	CFILE_CHAR	*ReadLn() {
		...
		CRoseArray<CFILE_CHAR> *result = new CRoseArray<CFILE_CHAR>
		(CFILE_MAXLINELENGHT);
		result->Clean();
	...
	*/
	CFILE_UINT Write(void *ptr, CFILE_SIZE element_size,
	                 CFILE_SIZE element_count = 1) {
		if (fh == NULL) {
			DoError(FE_NOFILEOPENED);
			return 0;
		...
		return (CFILE_UINT)(fwrite(ptr, (size_t)(element_size), (size_t)(element_count),
		                           fh));
	}
	/** Writes to the file starting at [offset] bytes relative to [origin]
	    ...
	    unsigned int indicating the number of bytes being written
	*/
	CFILE_UINT WriteFrom(void *ptr, CFILE_SIZE element_size,
	                     CFILE_SIZE element_count, CFILE_SLONG offset, signed int origin) {
		if (fh == NULL) {
			DoError(FE_NOFILEOPENED);
			return 0;
		}
...
#ifndef CFILE_NODEPRECATE
			size_t tmp;
			mbstowcs_s(&tmp, buffer, (size_t)(str->size()), str->arr,
			           (size_t)(str->size()));
#else
