...
QUESTCOND(001) {
	...
	for (int i = 0; i < data->iDataCnt; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Check Quest Var[%04x][%i] - %i (Op: %i)",
...
QUESTCOND(003) {
	...
	for (int i = 0; i < data->iDataCnt; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Check Stat[%i] - %i (Op: %i)", curAbil->iType,
		...
		switch (curAbil->iType) {
			...
			case sEXPRate:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Stats->xprate,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
...
QUESTCOND(004) {
	...
	for (int i = 0; i < data->iDataCnt; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Check Item - [Slot:%i][Type:%i]", curItem->iWhere,
			               curItem->uiItemSN);
		}
		...
		if (curItem->iWhere == 13) {
			...
			for (unsigned int k = 0; k < 5; k++)
				if (client->quest.quests[j].Items[k].itemnum == (curItem->uiItemSN % 1000)) {
					if (client->questdebug) {
						server->SendPM(client, "Operate - btOp: %i Val1: %i Val2: %i", curItem->btOp,
						               client->quest.quests[j].Items[k].count, curItem->iRequestCnt);
					}

					if (!OperateValues<int>(curItem->btOp,
					                        (int *)(&client->quest.quests[j].Items[k].count), curItem->iRequestCnt)) {
						return QUEST_FAILURE;
					} else {
						return QUEST_SUCCESS;
					}
...
QUESTCOND(008) {
	...
	if (client->questdebug) {
		server->SendPM(client, "QuestTime - ulTime: %i btOp: %i", data->ulTime,
		               data->btOp);
	}
	...
	if (server->STB_QUEST.rows[client->quest.quests[client->CheckQuest].QuestID][1]
	        > 0) {
		Time += client->quest.quests[client->CheckQuest].StartTime; // Start time
		Time += server->STB_QUEST.rows[client->quest.quests[client->CheckQuest].QuestID][1]
		        * 10; // Time to finish
...
QUESTCOND(014) {
	...
	return (client->quest.GetFlag(data->nSN) == dataOp) ? QUEST_SUCCESS :
	       QUEST_FAILURE;
}
