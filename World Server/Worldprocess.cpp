...
bool CWorldServer::GiveExp(CMonster *thismon) {
	...
	for (UINT i = 0; i < thismon->PlayersDamage.size(); i++) {
		...
		CPlayer *thisclient = GetClientByCID(thisplayer->charid,
		                                     thismon->Position->Map);
		...
		if (thisplayer->damage > 0 && thisclient != NULL) {
			...
			unsigned int exp = (unsigned int)ceil((double)((thismon->thisnpc->exp *
			                                      thisplayer->damage) / (thismon->thisnpc->hp * thismon->thisnpc->level)));
			...
			thisclient->CharInfo->Exp +=  GetColorExp(thisclient->Stats->Level,
			                              thismon->thisnpc->level, exp);
	...
	for (int p = 0; p < PartyExp.size(); p++) {
		...
		for (int i = 0; i < thisparty->num; i++) {
			CPlayer *partyclient = GetClientByCID(thisparty->partymember[i],
			                                      thismon->Position->Map);
			...
			if (!thisparty->flag) {
				...
				if (partyclient->Party->party->Exp > GetMaxPartyExp(
				            partyclient->Party->party->PartyLevel)) {
					partyclient->Party->party->PartyLevel++;
					partyclient->Party->party->Exp -= GetMaxPartyExp(
					                                      partyclient->Party->party->PartyLevel - 1);
				}
			...
			if (expoption == 0) {
				partyclient->CharInfo->Exp +=  GetColorExp(partyclient->Stats->Level,
				                               thismon->Stats->Level, (UINT)round(thisparty->exp / thisparty->num));
			} else {
				partyclient->CharInfo->Exp +=  GetColorExp(partyclient->Stats->Level,
				                               thismon->Stats->Level, (UINT)round(partyclient->Stats->Level * thisparty->exp /
				                                       thisparty->maxlevel));
			}
