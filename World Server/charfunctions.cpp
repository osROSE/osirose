...
bool CCharacter::IsOnBattle() {
	...
	switch (Battle->atktype) {
		...
		case SKILL_ATTACK:
		case AOE_TARGET: {
			if (Battle->skilltarget != 0 && Battle->skillid != 0) {
				return true;
			}
		}
		break;
		...
		case SKILL_BUFF: {
			if (Battle->bufftarget != 0 && Battle->skillid != 0) {
				return true;
			}
		}
		break;
...
bool CCharacter::IsTargetReached(CCharacter *Enemy, CSkills *skill) {
	...
	float fightdistance = GServer->distance(Position->current,
...
void CCharacter::RefreshBuff() {
	...
	for (UINT i = 0; i < 30; i++) {
		...
		if (etime >= MagicStatus[i].Duration * CLOCKS_PER_SEC) {
			...
			bflag = true;
		} else if (MagicStatus[i].Buff == A_POISON
		           && etime > 1 * CLOCKS_PER_SEC) { //Do poison dmg every 1.5 seconds
