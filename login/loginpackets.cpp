...
bool CLoginServer::pakUserLogin(CLoginClient *thisclient, CPacket *P) {
	try {
		...
		MYSQL_ROW row;
		thisclient->username.assign((const char *)&P->Buffer, 32,
		                            (P->Size - 6 - 32) > 16 ? 16 : P->Size - 6 - 32);
		thisclient->password.assign((const char *)&P->Buffer, 0, 32);
		BEGINPACKET(pak, 0x708);
		MYSQL_RES *result =
		    DB->QStore("SELECT id,password,accesslevel,online FROM accounts WHERE username='%s'",
		...
		if (mysql_num_rows(result) == 1) {
			...
			if (res == 0) {
				if (atoi(row[3]) == 1) {
					...
					thisclient->SendPacket(&pak);
					DB->QFree();
					DB->QExecute("update accounts set login_count=1 WHERE username='%s'",
					             thisclient->username.c_str());
					return true;
				}
			...
			if (Config.CreateLoginAccount) {
				if (!DB->QExecute("INSERT into accounts (username,password,accesslevel) values ('%s','%s',100)",
...
bool CLoginServer::pakGetServers(CLoginClient *thisclient, CPacket *P) {
	try {
		...
		MYSQL_RES *result =
		    DB->QStore("SELECT id,name,connected,maxconnections FROM channels WHERE owner=%i",
		               servernum);
		...
		while ((row = mysql_fetch_row(result))) {
			...
			BYTE id = atoi(row[0]);
			BYTE status = ((100 * connected) / (maxconnections == 0 ? 1 : maxconnections)) &
			              0xff;
			ADDWORD(pak, id);
			ADDBYTE(pak, 0);
			ADDWORD(pak, status);
...
bool CLoginServer::pakGetIP(CLoginClient *thisclient, CPacket *P) {
	try {
		...
		if (!DB->QExecute("UPDATE accounts SET lastsvr=%i,lastip='%s' WHERE id=%i",
		                  channelnum, inet_ntoa(thisclient->clientinfo.sin_addr), thisclient->userid)) {
			return false;
		}

		MYSQL_RES *result =
		    DB->QStore("SELECT host,port,connected,maxconnections FROM channels WHERE id=%i",
		               servernum);
