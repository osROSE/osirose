...
bool CCharServer::pakDoIdentify(CCharClient *thisclient, CPacket *P) {
	...
	result = DB->QStore("SELECT username,lastsvr,accesslevel,platinum FROM accounts WHERE id=%i AND password='%s'",
	...
	result = DB->QStore("SELECT online FROM accounts WHERE username='%s'",
	                    thisclient->username);
...
bool CCharServer::pakGetCharacters(CCharClient *thisclient, CPacket *P) {
	...
	CCharacter chars[5];
	//        0       1      2    3      4      5     6      7
	result = DB->QStore("SELECT char_name,level,face,hairStyle,sex,classid,id,deletetime FROM characters WHERE account_name='%s'",
	                    thisclient->username);
	...
	for (unsigned k = 0; k < charnum; k++) {
		...
		//       0       1      2         3         4        5
		result = DB->QStore("SELECT itemnum,itemtype,refine,durability,lifespan,slotnum FROM items WHERE owner=%i AND slotnum<10",
		                    chars[k].id);
		...
		ADDBYTE(pak,
		        0x00);     //thisclient->platinum?0x01:0x00 );					// IS PLATINUM?  00-NO;01-YES;02-YES BUT USER IS NOT
...
bool CCharServer::pakRequestWorld(CCharClient *thisclient, CPacket *P) {
	...
	result = DB->QStore("SELECT clanid,clan_rank,id,level,classid FROM characters WHERE char_name='%s'",
	                    thisclient->charname);
...
bool CCharServer::pakCreateChar(CCharClient *thisclient, CPacket *P) {
	...
	if (!DB->QExecute("INSERT INTO characters (account_name, char_name, face, hairStyle, sex) VALUES('%s','%s',%i,%i,%i)",
	...
	if (sex == 0) {
		if (!DB->QExecute("INSERT INTO items VALUES(%i,222,2,0,40,100,12,1,0,0,0,0)",
		                  mid)) {
			return false;
		}
	} else {
		if (!DB->QExecute("INSERT INTO items VALUES(%i,221,2,0,40,100,12,1,0,0,0,0)",
		                  mid)) {
			return false;
		}
	}
