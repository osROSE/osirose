...
AIACT(024) {
	...
	GETAIACTDATA(024);
	Log(MSG_DEBUG, "Do Skill btTarget %i nSkill %i nMotion %i", data->btTarget,
	    data->nSkill, data->nMotion);
...
AIACT(030) {
	...
	Log(MSG_INFO, "Execute Quest Trigger %s[%d] [%08x]", tempName,
	    data->lenszTrigger, hash);
