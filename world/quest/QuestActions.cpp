...
dword GetRewardValue(dword function, dword amount, CPlayer *client,
                     word nDupCNT) {
	switch (function) {
		case 0: {
			dword tempVal = amount + 0x1E;
			tempVal *= client->Attr->Cha +
			           0x0a; // We need to add a way to get all stats totals (Including buff, E, etc) - Drakia
...
QUESTREWD(001) {
	...
	if (client->questdebug) {
		server->SendPM(client, "Update Quest Item (uiItemSN: %u btOp: %u nDupCNT: %u)",
		               data->uiItemSN, data->btOp, data->nDupCNT);
	}
...
QUESTREWD(002) {
	...
	for (dword i = 0; i < data->iDataCnt; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Set quest var[%#04x][%i] - %i (Op: %i)",
			               curQst->m_wVarTYPE, curQst->m_wVarNO, curQst->nValue, curQst->btOp);
		}
...
QUESTREWD(003) {
	...
	for (dword i = 0; i < data->iDataCnt; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Update stat %i - %i (Op: %i)", curAbil->iType,
			               curAbil->iValue, curAbil->btOp);
		}

		switch (curAbil->iType) {
			case sGender:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->CharInfo->Sex,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sJob: {
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->CharInfo->Job,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sUnion:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Union_s->unionvar[0],
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sUnionPoints:
				if (!OperateValues<int>(curAbil->btOp,
				                        (int *)&client->Union_s->unionvar[client->Union_s->unionvar[0]],
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sStrength:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Str,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sDexterity:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Dex,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sIntelligence:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Int,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sConcentration:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Con,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sCharm:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Cha,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sSensibility:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Attr->Sen,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sLevel:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->Stats->Level,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sStatPoints:
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->CharInfo->StatPoints,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
			...
			case sMoney: {
				if (!OperateValues<int>(curAbil->btOp, (int *)&client->CharInfo->Zulies,
				                        curAbil->iValue)) {
					return QUEST_FAILURE;
				}
...
QUESTREWD(005) {
	...
	switch (data->btTarget) {
		case 0: { //EXP
			...
			client->CharInfo->Exp += GetRewardValue(data->btEquation, data->iValue, client,
			                                        0);
		...
		case 1: { //Zuly
			...
			client->CharInfo->Zulies += GetRewardValue(data->btEquation, data->iValue,
			                            client, 1);//dunno nDupCount for this one!
			BEGINPACKET(pak, 0x71D);
			ADDQWORD(pak, client->CharInfo->Zulies);
			client->client->SendPacket(&pak);
		}
		...
		case 2: { //Item
			...
			if (client->questdebug) {
				server->SendPM(client, "Give item [%i][%i]x%i", nItem.itemtype, nItem.itemnum,
				               nItem.count);
			}

			nItem.durability = GServer->STB_ITEM[nItem.itemtype -
			                                     1].rows[nItem.itemnum][29];
			nItem.gem = 0;
			nItem.stats = 0;
			nItem.refine = 0;
			nItem.socketed = 0;
...
QUESTREWD(006) {
	...
	if (client->questdebug) {
		server->SendPM(client, "Restore %i%% HP, %i%% MP", data->iPercentOfHP,
		               data->iPercentOfMP);
	}

	client->Stats->HP = (long int)((float)client->Stats->MaxHP / 100.0f) *
	                    data->iPercentOfHP;
	client->Stats->MP = (long int)((float)client->Stats->MaxHP / 100.0f) *
	                    data->iPercentOfMP;
	return QUEST_SUCCESS;
}
...
QUESTREWD(007) {
	...
	if (client->questdebug) {
		server->SendPM(client, "Teleport [%i][%f][%f]", data->iZoneSN, thispoint.x,
		               thispoint.y);
	}
...
QUESTREWD(008) {
	...
	for (dword i = 0; i < data->iHowMany; i++) {
		...
		if (client->questdebug) {
			server->SendPM(client, "Spawn mob[%i] @ %f, %f", data->iMonsterSN, pos.x,
			               pos.y);
		}
		...
		if (data->iMonsterSN > 750
		        && data->iMonsterSN < 755) { // ghost rackies = non aggresive
...
QUESTREWD(009) {
	...
	if (client->questdebug) {
		server->SendPM(client, "Execute Quest Trigger %s[%d] [%08x]", tempName,
		               data->shNameLen, hash);
	}
...
QUESTREWD(023) {
	...
	GServer->DB->QExecute("UPDATE list_clan SET grade=%i WHERE id=%i",
	                      client->Clan->grade, client->Clan->clanid);
	return QUEST_SUCCESS;
}
...
QUESTREWD(028) {
	...
	if (client->questdebug) {
		server->SendPM(client, "Clan teleport to [%i][%f][%f]", data->nZoneNo,
		               telepos.x, telepos.y);
	}
