...
bool CMonster::PlayerInRange() {
	...
	for (int i = 0; i < map->PlayerList.size(); i++) {
		...
		if (GServer->IsVisible(thisclient, this)) {
			return true;
		}
	}
...
bool CMonster::CanMove() {
	if ((montype < 41 || montype > 45) && (montype < 325 || montype > 329)
	        && montype != 659 && (montype < 771 || montype > 810) && montype != 992
	        && (montype < 1474 || montype > 1489)) {
		return true;
	}
