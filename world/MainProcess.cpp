		...
		for (UINT i = 0; i < GServer->MapList.Map.size(); i++) {
			...
			for (UINT j = 0; j < map->MonsterList.size(); j++) {
				...
				*/
				...
				if (monster->hitcount ==
				        0xFF) { //this is a delay for new monster spawns this might olso fix invisible monsters(if they attack directly on spawning the client dosn't get the attack packet(its not in it's visible list yet))
				...
				if (monster->montype > 750 && monster->montype < 754) { //ghost rackie
					...
					if (etime > 5000) {
						...
						Log(MSG_INFO,
						    "Monster type %i detected after 5 seconds. Changing to monster type %i",
						    monster->montype, newmon);
				...
				} else if (/*!monster->IsMoving()&&*/!monster->IsAttacking()
				                                     && !monster->IsDead()) {
					if (2000 < (UINT)GServer->round((clock() - monster->lastAiUpdate))) {
