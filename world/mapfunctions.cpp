...
bool CMap::TeleportPlayer(CPlayer *player, fPoint coord, bool TelePassenger) {
	...
	if (player->Stats->HP < 1) {
		player->Stats->HP = player->Stats->MaxHP * 10 / 100;
	}
...
bool CMap::pakTeleport(CPlayer *player, unsigned int Map, fPoint coord,
                       bool TelePassenger) {
	GServer->MapList.Index[player->Position->Map]->RemovePlayer(player, false);
...
CRespawnPoint *CMap::GetNearRespawn(CPlayer *player) {
	...
	for (UINT i = 0; i < RespawnList.size(); i++) {
		float resdist = GServer->distance(player->Position->current,
		                                  RespawnList.at(i)->dest);
...
void CMap::UpdateTime() {
	...
	if (ctime >= morningtime && ctime < daytime) {
		CurrentTime = MORNING;
	} else if (ctime >= daytime && ctime < eveningtime) {
		CurrentTime = DAY;
	} else if (ctime >= eveningtime && ctime < nighttime) {
		CurrentTime = EVENING;
	} else if (ctime >= nighttime) {
		CurrentTime = NIGHT;
	}
...
void CMap::RespawnMonster() {
	...
	for (UINT k = 0; k < MonsterSpawnList.size(); k++) {
		...
		if (thisspawn->respawntime < 2) {
			thisspawn->respawntime = 2;
		}

		clock_t rtime = clock() - thisspawn->lastRespawnTime;

		if ((rtime > thisspawn->respawntime * CLOCKS_PER_SEC
		        && thisspawn->amon < thisspawn->max) || thisspawn->amon < thisspawn->min) {
			fPoint position = GServer->RandInPoly(thisspawn->points, thisspawn->pcount);
			AddMonster(thisspawn->montype, position, 0, thisspawn->mobdrop,
	...
	for (UINT j = 0; j < MobGroupList.size(); j++) {
		...
		if (rtime < thisgroup->respawntime * CLOCKS_PER_SEC
		        || thisgroup->active >= thisgroup->limit) {
			continue;
		}
		...
		for (UINT k = thisgroup->active; k < thisgroup->limit; k++) {
			...
			if (thisgroup->tacMobs.size() > 0
			        && thisgroup->basicKills >= thisgroup->tacticalpoints) {
			...
			for (UINT i = 0; i < thismob->amount; i++) {
				...
				AddMonster(thismob->mobId, position, 0, thismob->mobdrop, thismob->mapdrop,
				           thisgroup->id);
			}
		}
	}
