...
void CWorldServer::OnClientDisconnect(CClientSocket *thisclient) {
	...
	if (!player->Saved) {
		...
		ADDBYTE(pak, 0xfa);
		ADDWORD(pak,
		        player->Session->userid);      //CharInfo->charid );CharInfo->charid );
		ADDBYTE(pak, 0x00);
	...
	if (player->Fairy) {
		...
		GServer->FairyList.at(player->FairyListIndex)->WaitTime =
		    GServer->Config.FairyWait * (rand() % GServer->GetFairyRange(
		                                     1) + GServer->GetFairyRange(0));
		player->Fairy = false;
		player->FairyListIndex = 0;
		GServer->DoFairyStuff(player, 0);
		// recalculate FairyMax
		int oldFairyMax = GServer->Config.FairyMax;
		GServer->Config.FairyMax = (int)ceil((float)GServer->ClientList.size() /
		                                     50.0); //(1 fairy more every 50 player)

		if (oldFairyMax > GServer->Config.FairyMax) {
			GServer->FairyList.erase(GServer->FairyList.begin() +
			                         GServer->FairyList.size());
		}
	}
	...
	DB->QExecute("UPDATE accounts SET online=false where id=%u",
...
#else
				NewSocket = accept(sckISCII, (sockaddr *)&ClientInfo,
...
bool CWorldServer::OnServerReady() {
	...
	Log(MSG_INFO, "Process Loaded. WorldDelay %i | MapDelay %i | VisualDelay %i",
	    Config.WorldDelay, Config.MapDelay, Config.VisualDelay);
	...
	if (!DB->QExecute("INSERT INTO channels (id,name,host,port,connected,maxconnections,owner) VALUES (%i,'%s','%s',%u,0,%i,%i)",
	                  Config.ServerID, Config.ServerName, Config.WorldIP, Config.WorldPort,
	                  Config.MaxConnections, Config.ParentID)) {
		Log(MSG_WARNING,
		    "Error accessing to database, the other server will not connect to WorldServer");
	}
...
bool CWorldServer::SendLevelUPtoChar(CPlayer *thisclient) {
	...
	ADDWORD(pak,
	        thisclient->CharInfo->charid);  //Session->userid );//CharInfo->charid );
...
bool CWorldServer::pakGiveItem(CPlayer *thisclient, unsigned char itemtype,
                               unsigned short itemid, unsigned short count) {
	...
	if (itemtypes[thisitem.itemtype] == 0) {
		...
		thisitem.gem = 0;
		thisitem.durability = STB_ITEM[thisitem.itemtype -
		                               1].rows[thisitem.itemnum][29];
	}
...
void CWorldServer::DePopulateZone(CPlayer *thisclient, int zonex,
                                  int zoney) { //compile warning fix by Fluorescent abs -> fabs
	CPacket pakout, pakout2;

	if (!thisclient->Session->inGame
	        || fabs(thisclient->Position->current.x - zonex) > 1
	        || fabs(thisclient->Position->current.y - zoney) > 1) {
		...
		for (unsigned i = 0; i < ClientList.size(); i++) {
			...
			if (otherclient->Position->current.x == zonex
			        && otherclient->Position->current.y == zoney
			        && otherclient->Position->Map == thisclient->Position->Map) {
		...
		for (unsigned i = 0; i < MapList.Map.size(); i++) {
			...
			for (UINT j = 0; j < thismap->MonsterSpawnList.size(); j++) {
				...
				if (thismonster->Position->current.x == zonex
				        && thismonster->Position->current.y == zoney
				        && thismonster->Position->Map == thisclient->Position->Map) {
				...
				for (unsigned j = 0; j < thismap->NPCList.size(); j++) {
					...
					if (thisnpc->pos.x == zonex && thisnpc->pos.y == zoney
				...
				for (unsigned j = 0; j < thismap->DropsList.size(); j++) {
					...
					if (thisdrop->pos.x == zonex && thisdrop->pos.y == zoney
...
bool CWorldServer::pakTeleport(CPlayer *thisclient, unsigned short map, float x,
                               float y) {
	fPoint coord;
	coord.x = x / 100;
	coord.y = y / 100;
...
void CWorldServer::LoadConfigurations(char *file) {
	...
	Config.SQLServer.pcDatabase = ConfigGetString(file, "mysql_database",
	                              "roseon_beta");
	...
	Config.WELCOME_MSG          = ConfigGetString(file, "welcome_msg",
...
void CWorldServer::LoadCommandLevels(void) {
	...
	Config.Command_ChangeFairyWait = ConfigGetInt("commands.ini", "changefairywait",
	                                 299);
	Config.Command_ChangeFairyStay = ConfigGetInt("commands.ini", "changefairystay",
	                                 299);
	Config.Command_ChangeFairyTestMode = ConfigGetInt("commands.ini",
	                                     "changefairytestmode", 299);
	...
	Config.Command_GlobalPrefix = ConfigGetString("commands.ini", "globalprefix",
	                              "[Broadcast]");
}
...
void CWorldServer::ReceivedISCPacket(CPacket *pak) {
	switch (pak->Command) {
		...
		default:
			Log(MSG_DEBUG, "UNKNOWN ISC PACKET - 0x%x %04x %04x", pak->Command, pak->Size,
			    pak->Unused);
	}
}
...
bool CWorldServer::OnReceivePacket(CClientSocket *thisclient, CPacket *P) {
	...
	switch (P->Command) {
		case 0x0500:
			return pakCSReady((CPlayer *)thisclient->player, P);

		case 0x0502:
			return pakCharDSClient((CPlayer *)thisclient->player, P);

		case 0x0505:
			return pakCSCharSelect((CPlayer *)thisclient->player, P);

		case 0x0700:
			return pakPing((CPlayer *)thisclient->player, P);

		case 0x0707:
			return pakExit((CPlayer *)thisclient->player, P);

		case 0x070b:
			return pakDoIdentify((CPlayer *)thisclient->player, P);

		case 0x071c:
			return pakCharSelect((CPlayer *)thisclient->player, P);

		case 0x0730:
			return pakQuestTrigger((CPlayer *)thisclient->player, P);

		case 0x0753:
			return pakDoID((CPlayer *)thisclient->player, P);

		case 0x0755:
			return pakUserDied((CPlayer *)thisclient->player, P);

		case 0x0756:
			return pakSaveTown((CPlayer *)thisclient->player, P);

		case 0x0762:
			return pakWeight((CPlayer *)thisclient->player, P);

		case 0x0771:
			return pakStopChar((CPlayer *)thisclient->player, P);

		case 0x0781:
			return pakDoEmote((CPlayer *)thisclient->player, P);

		case 0x0782:
			return pakChangeStance((CPlayer *)thisclient->player, P);

		case 0x0783:
			return pakNormalChat((CPlayer *)thisclient->player, P);

		case 0x0784:
			return pakWhisper((CPlayer *)thisclient->player, P);

		case 0x0785:
			return pakShout((CPlayer *)thisclient->player, P);

		case 0x0786:
			return pakPartyChat((CPlayer *)thisclient->player, P);

		case 0x0798:
			return pakStartAttack((CPlayer *)thisclient->player, P);

		case 0x079f:
			return pakShowHeal((CPlayer *)thisclient->player, P);

		case 0x079a:
			return pakMoveChar((CPlayer *)thisclient->player, P);

		case 0x07a1:
			return pakNPCBuy((CPlayer *)thisclient->player, P);

		case 0x07a3:
			return pakUseItem((CPlayer *)thisclient->player, P);

		case 0x07a4:
			return pakDoDrop((CPlayer *)thisclient->player, P);

		case 0x07a5:
			return pakChangeEquip((CPlayer *)thisclient->player, P);

		case 0x07a7:
			return pakPickDrop((CPlayer *)thisclient->player, P);

		case 0x07a8:
			return pakGate((CPlayer *)thisclient->player, P);

		case 0x07a9:
			return pakAddStats((CPlayer *)thisclient->player, P);

		case 0x07aa:
			return pakMoveSkill((CPlayer *)thisclient->player, P);

		case 0x07ab:
			return pakEquipABC((CPlayer *)thisclient->player, P);

		case 0x07af:
			return pakCraft((CPlayer *)thisclient->player, P);

		case 0x07b1:
			return pakLevelUpSkill((CPlayer *)thisclient->player, P);

		case 0x07b2:
			return pakSkillSelf((CPlayer *)thisclient->player, P);

		case 0x07b3:
			return pakStartSkill((CPlayer *)thisclient->player, P);

		case 0x07b4:
			return pakSkillAOE((CPlayer *)thisclient->player, P);

		case 0x07c0:
			return pakTradeAction((CPlayer *)thisclient->player, P);

		case 0x07c1:
			return pakTradeAdd((CPlayer *)thisclient->player, P);

		case 0x07ad:
			return pakStorage((CPlayer *)thisclient->player, P);

		case 0x07ae:
			return pakChangeStorage((CPlayer *)thisclient->player, P);

		case 0x07ba:
			return pakidentify((CPlayer *)thisclient->player, P);

		case 0x07bc:
			return pakModifiedItem((CPlayer *)thisclient->player, P);

		case 0x07bf:
			return true;//add to wishlist

		case 0x07c2:
			return pakOpenShop((CPlayer *)thisclient->player, P);

		case 0x07c3:
			return pakCloseShop((CPlayer *)thisclient->player, P);

		case 0x07c4:
			return pakShowShop((CPlayer *)thisclient->player, P);

		case 0x07c5:
			return pakBuyShop((CPlayer *)thisclient->player, P);

		case 0x07c6:
			return pakSellShop((CPlayer *)thisclient->player, P);

		case 0x07ca:
			return pakChangeCart((CPlayer *)thisclient->player, P);

		case 0x07cb:
			return pakRepairHammer((CPlayer *)thisclient->player, P);

		case 0x07cd:
			return pakRepairItem((CPlayer *)thisclient->player, P);

		case 0x07d0:
			return pakPartyActions((CPlayer *)thisclient->player, P);

		case 0x07d1:
			return pakPartyManager((CPlayer *)thisclient->player, P);

		case 0x07d7:
			return pakPartyOption((CPlayer *)thisclient->player, P);

		case 0x07d8:
			return pakModifiedItemDone((CPlayer *)thisclient->player, P);

		case 0x07d9:
			return pakItemMall((CPlayer *)thisclient->player, P);

		case 0x07da:
			return pakStoreZuly((CPlayer *)thisclient->player, P);

		case 0x07dd:
			return pakRideRequest((CPlayer *)thisclient->player, P);

		case 0x07e0:
			return pakCreateClan((CPlayer *)thisclient->player, P);

		case 0x07e1:
			return pakClanManager((CPlayer *)thisclient->player, P);

		case 0x07eb:
			return pakPrintscreen((CPlayer *)thisclient->player, P);

		case 0x0808:
			return pakGameGuard((CPlayer *)thisclient->player, P);

		default:
			Log(MSG_WARNING, "(SID:%i) Received unknown packet. Command:%04x Size:%04x",
