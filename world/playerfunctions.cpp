...
bool CPlayer::GetPlayerInfo() {
	char text[50];
	sprintf(text, "Attack: %i | Critical: %i", Stats->Attack_Power,
	        Stats->Critical);
	...
	ADDBYTE(pak, 0);
	client->SendPacket(&pak);
	sprintf(text, "Defense: %i | Magic Defense: %i", Stats->Defense,
	        Stats->Magic_Defense);
	...
	ADDBYTE(pak, 0);
	client->SendPacket(&pak);
	sprintf(text, "aspeed: %i | mspeed: %i", Stats->Attack_Speed,
	        Stats->Move_Speed);
	RESETPACKET(pak, 0x0784);
	ADDSTRING(pak, "[GM]PlayerInfo");
	ADDBYTE(pak, 0);
	...
	ADDBYTE(pak, 0);
	client->SendPacket(&pak);
	sprintf(text, "HP: %i/%i , MP: %i/%i", Stats->HP, Stats->MaxHP, Stats->MP,
	        Stats->MaxMP);
	RESETPACKET(pak, 0x0784);
	ADDSTRING(pak, "[GM]PlayerInfo");
	ADDBYTE(pak, 0);
	...
	ADDBYTE(pak, 0);
	client->SendPacket(&pak);
	sprintf(text, "Position[%i]: (%.0f,%.0f)", Position->Map, Position->current.x,
	        Position->current.y);
	RESETPACKET(pak, 0x0784);
	ADDSTRING(pak, "[GM]PlayerInfo");
	ADDBYTE(pak, 0);
	...
	ADDBYTE(pak, 0);
	client->SendPacket(&pak);
	sprintf(text, "ClanName[%u]: %s | ClanGrade: %i | ClanRank: %i", Clan->clanid,
	        Clan->clanname, Clan->grade, Clan->clanrank);
	RESETPACKET(pak, 0x0784);
	ADDSTRING(pak, "[GM]PlayerInfo");
	ADDBYTE(pak, 0);
	ADDSTRING(pak, text);
...
bool CPlayer::VisiblityList() {
	...
	for (UINT i = 0; i < map->PlayerList.size(); i++) {
		...
		float distance = GServer->distance(this->Position->current,
		                                   otherclient->Position->current);
	...
	*/
	...
	if (itemtab == 1 || itemtab == 2) {
		for (int i = 0; i < 30; i++) {
			...
			if (items[slot].itemnum == thisitem.itemnum
			        && items[slot].itemtype == thisitem.itemtype
			        && (items[slot].count + thisitem.count) < 999) {
				return slot;
			}
...
UINT CPlayer::GetNewStorageItemSlot(CItem thisitem) {
	...
	if (thisitem.itemtype == 10 || thisitem.itemtype == 11
	        || thisitem.itemtype == 12) {
		for (UINT i = 0; i < 160; i++) {
			if (storageitems[i].itemtype == thisitem.itemtype
			        && storageitems[i].itemnum == thisitem.itemnum) {
...
int CPlayer::ExecuteQuestTrigger(dword hash) {
	...
	for (dword i = 0; i < trigger->ConditionCount; i++) {
		...
		success = (*GServer->qstCondFunc[command])(GServer, this,
		          trigger->Conditions[i]->data);
	...
	for (dword i = 0; i < trigger->ActionCount; i++) {
		...
		success = (*GServer->qstRewdFunc[command])(GServer, this,
		          trigger->Actions[i]->data);
...
void CPlayer::takeSkillCost(CSkills *skill) {
	for (int i = 0; i < 2; i++) {
		...
		switch (skill->costtype[i]) {
			...
			case sMP:
				Stats->MP -= (skill->costamount[i] - (skill->costamount[i] *
				                                      Stats->MPReduction / 100));
