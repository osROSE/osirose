...
bool CWorldServer::LoadStatusData() {
	...
	for (unsigned int i = 0; i < STB_STATUS.rowcount; i++) {
		...
		newstatus->repeat =
		    STB_STATUS.rows[i][4]; // 1: Repeat(Heal) 2: Once(Buff Stat) 3: Status Effect(Poison, etc)
...
bool CWorldServer::LoadSkillData() {
	...
	for (unsigned int i = 0; i < STB_SKILL.rowcount; i++) {
		...
		if (newskill->success == 0) {
			newskill->success =
			    100;    //success rate is stored as NULL where it is 100%. eg dual scratch
		}
		...
		newskill->costtype[1] =
		    STB_SKILL.rows[i][18]; // All driveby skills have 2 costs
		newskill->costamount[1] =
		    STB_SKILL.rows[i][19]; // The second is normally cart gauge
		newskill->cooldown =
		    STB_SKILL.rows[i][20];   // The time it takes to be able to use the skill again.
		//newskill->success=STB_SKILL.rows[i][69];    //Dunno what this field is really but it sure isn't success rate <_<
		newskill->weapon[0] =
		    STB_SKILL.rows[i][30];    // Item Type must be equipped(Only 1)
		...
		newskill->value2[2] = 0;
		newskill->req[0] =
		    STB_SKILL.rows[i][45];  //the requirement type(usually 31 = level)
		newskill->clevel = STB_SKILL.rows[i][46];  //requirement amount
		newskill->reqam[0] =
		    STB_SKILL.rows[i][46]; //this need no always be level but is usually
		newskill->req[1] =
		    STB_SKILL.rows[i][47];  //the requirement type(in most cases blank)
		...
		if ((STB_SKILL.rows[i][8]) >
		        0) { //this boolean is kind of pointless too. Not all skills with AOE range are actually designated as AOE. We should use skilltype instead.
			newskill->aoe = 1;
...
bool CWorldServer::LoadTeleGateData() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`, `srcx`, `srcy`, `srcmap`, `destx`, `desty`, `destmap` FROM `list_telegates`");
...
bool CWorldServer::LoadRespawnData() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`,`x`,`y`,`map`,`radius`,`type` FROM `list_respawnzones`");
	...
	while (row = mysql_fetch_row(result)) {
		...
		MapList.Index[thisrespawnpoint->destMap]->RespawnList.push_back(
		    thisrespawnpoint);
	}
...
bool CWorldServer::LoadMobGroups() {
	...
	char *tmp = NULL;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`, `map`, `x`, `y`, `range`, `respawntime`, `limit`, `tacticalpoints`, `moblist` FROM `list_mobgroups`");
...
bool CWorldServer::LoadMonsterSpawn() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`,`map`,`montype`,`min,max`,`respawntime`,`points` FROM `list_spawnareas`");
...
bool CWorldServer::LoadNPCs() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `type`,`map`,`dir`,`x`,`y`,`dialogid` FROM `list_npcs`");
	...
	while (!feof(fh)) {
		...
		for (UINT j = 0; j < newdrop->Drops.size(); j++) {
			...
			if (value == 0) {
				...
				Log(MSG_WARNING,
				    "Probability is not complete, dropid: %i - temporal probability will be 1 * rate",
				    newdrop->id);
			} else {
...
bool CWorldServer::LoadPYDropsData() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`,`type`,`min_level`,`max_level`,`prob`,`mob`,`map`,`alt` FROM `item_drops`");
...
bool CWorldServer::LoadSkillBookDropsData() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `id`,`itemtype`,`min`,`max`,`prob` FROM `list_skillbooks`");
...
bool CWorldServer::LoadConfig() {
	...
	MYSQL_ROW row;
	MYSQL_RES *result =
	    DB->QStore("SELECT `exp_rate`, `drop_rate`, `zuly_rate`, `blue_chance`, `stat_chance`, `slot_chance`, \
	...
	for (UINT i = 0; i < MapList.Map.size(); i++) {
		...
		for (UINT j = 0; j < thismap->MonsterSpawnList.size(); j++) {
			...
			for (UINT k = 0; k < thisspawn->max; k++) {
				...
				thismap->AddMonster(thisspawn->montype, position, 0, thisspawn->mobdrop,
				                    thisspawn->mapdrop, thisspawn->id);
			}
		}
	}
		...
		for (UINT j = 0; j < thismap->MobGroupList.size(); j++) {
			...
			for (UINT k = 0; k < thisgroup->limit; k++) {
				...
				for (UINT l = 0; l < thismob->amount; l++) {
					...
					thismap->AddMonster(thismob->mobId, position, 0, thismob->mobdrop,
					                    thismob->mapdrop, thisgroup->id);
				}
			}
		}
	}
	...
	for (int j = 0; j < 9; j++) {
		for (unsigned int i = 0; i < STB_ITEM[j].rowcount; i++) {
			...
			if (STB_ITEM[j].fieldcount > 35) {
				...
				newequip->magicattack =
				    STB_ITEM[j].rows[i][37];//new. Staffs and wands are designated as magic weapons.

				if (newequip->magicattack > 1) {
					newequip->magicattack =
					    0; //all the blue named spears have weird numbers here. Maybe why NA had a massive damage glitch with them.
				}
			} else {
			...
			if (newequip->id > 4999) {
				...
				EquipList[newequip->equiptype].Index[newequip->id] =
				    newequip; // Index to read more quickly the data
			}
...
bool CWorldServer::LoadPatItem() {
	...
	for (unsigned int i = 0; i < STB_ITEM[13].rowcount; i++) {
		...
		newpat->level =
		    STB_ITEM[13].rows[i][22];             // extra field added rev 70
		newpat->condition[1] =
		    STB_ITEM[13].rows[i][24];      // extra field added rev 70
		newpat->modifier[1] =
		    STB_ITEM[13].rows[i][25];       // extra field added rev 70
		newpat->condition[2] =
		    STB_ITEM[13].rows[i][27];      // extra field added rev 70
		newpat->modifier[2] =
		    STB_ITEM[13].rows[i][28];       // extra field added rev 70
