	...
	switch (Battle->atktype) {
		case NORMAL_ATTACK: { //normal attack
			...
			if (IsTargetReached(Enemy) && CanAttack()) {
				...
				if (Enemy->IsMonster()) {
					CMonster *monster = GServer->GetMonsterByID(Enemy->clientid,
					                    Enemy->Position->Map);
...
void CCharacter::NormalAttack(CCharacter *Enemy) {
	...
	if (hitvalue < 20)
		if (int((float(Stats->Level - Enemy->Stats->Level) * 0.6f) + float((
		            rand() % 100) + 1)) >= 94) {
			hitvalue = 100;
		}
...
bool CCharacter::AoeSkill(CSkills *skill, CCharacter *Enemy) {
	...
	for (UINT i = 0; i < map->MonsterList.size(); i++) {
		...
		if (GServer->IsMonInCircle(Position->skilltargetpos, monster->Position->current,
		                           (float)skill->aoeradius + 1)) {
	...
	if (map->allowpvp != 0 || (IsMonster() && !IsSummon())) {
		for (UINT i = 0; i < map->PlayerList.size(); i++) {
			...
			if (GServer->IsMonInCircle(Position->skilltargetpos, player->Position->current,
			                           (float)skill->aoeradius + 1)) {
	...
	if (CharType == 1) {
			...
			switch (skill->target) {
				case tPartyMember: // party
					if (player->Party->party == GetParty()) {
						if (GServer->IsMonInCircle(Position->current, player->Position->current,
						                           (float)skill->aoeradius + 1)) {
		...
		for (UINT i = 0; i < map->MonsterList.size(); i++) {
			...
			switch (skill->target) {
				...
				case tAllCharacters:
				case tAllMembers: { // all characters
					if (GServer->IsMonInCircle(Position->current, monster->Position->current,
					                           (float)skill->aoeradius + 1)) {
...
void CCharacter::UseAtkSkill(CCharacter *Enemy, CSkills *skill, bool deBuff) {
	...
	*/
	...
	long int apchar =
	    Stats->Attack_Power; //apchar neccesary, because if i use stats->Attack_Power will change normal attack code
	...
	if (skill->skilltype == 6 || skill->skilltype == 7) {
		skillpower = (long int)((skillpower + apchar * 2 + (Stats->Attack_Power -
		                         Enemy->Stats->Magic_Defense) - Enemy->Stats->Magic_Defense * 2 + level_diff) /
		                        1.8);
		skillpower += GServer->RandNumber(0, 20);
	} else {
		skillpower = (long int)((skillpower + apchar * 2 + (Stats->Attack_Power -
		                         Enemy->Stats->Defense) - Enemy->Stats->Magic_Defense * 2 + level_diff) / 2.4);
		skillpower += GServer->RandNumber(0, 20);
	}
	...
	if (skillpower >= 5000000) {
		skillpower =
		    5;    //if ever computations got wild- this will avoid having crazy damage -littlelilly
	}

	if (skill->atkpower <= 5) {
		skillpower =
		    0;    //so that skills having 0 attack have no damage at all - littlelilly
	}
	...
	if (Enemy->IsDead()) {
		...
		bflag = GServer->AddBuffs(skill, Enemy,
		                          GetInt());    // send to AddBuffs instead.
...
void CCharacter::UseBuffSkill(CCharacter *Target, CSkills *skill) {
	...
	if (skill->nbuffs > 0 && bflag == true) {
		...
		if (skill->nbuffs > 1) { //???????????
			ADDWORD(pak, Battle->skillid + 0x3000 + ((GetInt() % 4) *
			        0x4000)); //maybe number of buffs?
		} else {
			ADDWORD(pak, (Battle->skillid + 0x1000) + ((GetInt() % 4) * 0x4000));
		}
	...
	for (int i = 0; i < 2; i++) {
		...
		************************/
		if (status->repeat == 1) { // Continuous
			Log(MSG_INFO, "Need to take stat %i and %s it by %i%s over %i seconds",
			    skill->buff[i],
			...
			Log(MSG_INFO, "Need to take stat %i and %s it by %i%s for %i seconds",
			    skill->buff[i],
			    ((status->decrease) ? "decrease" : "increase"),
			    ((skill->value1[i] != 0) ? skill->value1[i] : skill->value2[i]),
			    ((skill->value1[i] != 0) ? "" : "%"), skill->duration);
		} else if (status->repeat == 3) { // Status Effect (Poison,etc)
			Log(MSG_INFO, "Need to give user status effect %i for %i seconds",
			    skill->buff[i],
			    ((skill->value1[i] != 0) ? skill->value1[i] : skill->value2[i]),
			    (skill->duration));
		}
...
int CCharacter::GetAtkDmg(CCharacter *pDEF, word wHitCNT, int iSuc) {
	...
	int unk1 = (((int)GetAttackPower() + 20) * (28 - critSuccessRate)) / ((
	               int)pDEF->GetDefense() + 5);
	...
	if (critSuccessRate >= 20) {
		if (IsPlayer() && pDEF->IsPlayer()) {
			...
			curDamage = (int)(tempFloat / ((((int)pDEF->GetDodge() * 0.4f) +
			                                (int)pDEF->GetDefense() + 5.0f) * 420.0f) + 20);
		} else {
			float tempFloat = ((iSuc * 0.03f) + 26) * (int)GetAttackPower();
			tempFloat *= (int)GetAttackPower() - (int)pDEF->GetDefense() + 250;
			curDamage = (int)(tempFloat / ((((int)pDEF->GetDodge() * 0.4f) +
			                                (int)pDEF->GetDefense() + 5.0f) * 145.0f));
		}
		...
		*/
		...
		if (IsPlayer() && pDEF->IsPlayer()) {
			...
			curDamage = (int)(tempFloat / ((((int)pDEF->GetDodge() * 0.4f) +
			                                (int)pDEF->GetDefense() + 10) * 300.0f) + 25);
		} else {
			float tempFloat = ((iSuc * 0.05f) + 29.0f) * (int)GetAttackPower();
			tempFloat *= (((int)GetAttackPower() - (int)pDEF->GetDefense()) + 230);
			curDamage = (int)(tempFloat / ((((int)pDEF->GetDodge() * 0.3f) +
			                                (int)pDEF->GetDefense() + 5) * 100.0f));
		}
...
int CCharacter::GetMgcDmg(CCharacter *pDEF, word wHitCNT, int iSuc) {
	int curDamage = 0;
	int critSuccessRate = GetCritRate();
	int unk1 = ((GetAttackPower() + 20) * (28 - critSuccessRate)) /
	           (pDEF->GetMagicDefense() + 5);
